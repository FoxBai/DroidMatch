...
- 上述 214 项是 setup fail-closed 收口时的阶段库存；继续加入分页/scan 上限、
  ...
  当前 Android JVM 单元测试库存为 296 项。最大 1000 行页面会保持顺序、只消费
...
ADB endpoint 可同时服务多个 loopback 会话，但共享 `DmFileProvider` 会为 active upload、partial cleanup 与 mutation 持有统一的逻辑路径 claim。第二个同目标 open、命中上传目标的 rename/delete，或命中其目录祖先的 mutation 会立即返回 `ERROR_CODE_ALREADY_EXISTS`；无交集路径仍可并行。open 失败、write abort、final commit、cancel/close、mutation 退场和 session teardown 都会释放 claim。
