...
- `DmFileProvider`：负责 M1 root、SAF process-local token cache 与 catalog 路由；`ProviderPathRouter` 负责 logical path/target，`ProviderPagePolicy` 独立负责 query-bound opaque page token、分页上限和默认排序；`AndroidAppSandboxCatalog` 负责 canonical app-private 文件系统，并以流式扫描和至多 `offset + pageSize` 个候选完成稳定排序分页，不再复制完整目录数组；`AndroidMediaCatalog` 负责动态媒体权限与 MediaStore，`AndroidSafCatalog` 负责 persisted tree permission、document query/page/download 和 transfer-ID partial resume；`ProviderDownloadReaders` / `ProviderUploadWriters` 分别拥有传输读取与提交/清理状态，共享 helper 统一 ID、MIME 和 error-path cleanup。
