...
- launcher 入口：安装后启动器中显示 DroidMatch 图标，打开 `DroidMatchActivity` 完成安全连接、配对、通知、照片/视频与 SAF 授权管理；它仍不是完整文件管理器。本轮媒体授权/重选只有 JVM、assemble 与 lint 证据，尚无产品 UI 真机归档。
