...
- 音乐入口使用实时音频授权：API 33+ 单独请求 `READ_MEDIA_AUDIO`，API 26–32 沿用已有存储读取权限；选中照片/视频不授权音频。`dm://media-audio/` 返回音频列表和可用时长，API 29+ 可向 `Music/DroidMatch` 新建音频；旧版使用可写 SAF。仅本地验证，见[基础音乐](../docs/basic-music.md)。
