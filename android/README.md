...
- 音乐入口使用实时音频授权：API 33+ 单独请求 `READ_MEDIA_AUDIO`，API 26–32 沿用已有存储读取权限；选中照片/视频不授权音频。`dm://media-audio/` 返回音频列表和可用时长，API 29+ 可向 `Music/DroidMatch` 新建音频；旧版使用可写 SAF。API 29+ 可通过有界 typed image 派生物提供[音乐封面](../docs/music-artwork.md)，逐次读取及发布前复核音频权限；旧版或无封面时返回不可用。仅本地验证，见[基础音乐](../docs/basic-music.md)。
