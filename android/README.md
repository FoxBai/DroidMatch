...
`RpcDispatcher` 会把 `OpenTransferRequest.transfer_id` 传到 provider upload 层；当前 app-sandbox/MediaStore/SAF 行为不变，但 SAF 后续实现 durable partial document key、retry/resume 状态时必须使用这颗稳定 transfer id，而不是从用户可见 display name 推导。
