...
- `RpcDispatcher`：同一 session 先处理 `ClientHello`，再处理 `DeviceInfoRequest`、`ListDirRequest` 和 `DiagnosticsRequest`。
...
- `DmFileProvider`：提供 M1 `dm://roots/` 虚拟根目录，通过 MediaStore 列出 `dm://media-images/` / `dm://media-videos/`，并列出已授权 `dm://saf-.../` root 的首层/子目录内容。
...
当前还没有传输 RPC 分发。启动服务和指定 Android 端口的真机流程仍会继续收口。
