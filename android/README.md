...
- `DmFileProvider`：负责 M1 `dm://roots/` 与 provider logical path 路由；`AndroidAppSandboxCatalog` 独立负责 app-owned root 下的 canonical path、防目录穿越、listing/sort/page 和 partial 隐藏，`ProviderDownloadReaders` 独立拥有 seekable FD、顺序 stream offset fallback、跨 chunk/EOF/final/close 状态，`ProviderUploadWriters` 独立拥有 app-sandbox 原子提交/partial 保留、SAF 临时文档重命名/清理和 MediaStore pending row 发布/清理，`ProviderOpaqueIds` 统一生成不泄露原始路径的稳定 logical ID。
