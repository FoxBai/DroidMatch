...
- `RpcDispatcher`：同一 session 先处理 `ClientHello`，再处理 `HeartbeatRequest`、`DeviceInfoRequest`、`ListDirRequest`、`OpenTransferRequest(download)` 多 chunk 发送、resume source fingerprint 校验、`TransferChunkAck`、`CancelTransferRequest`、`PauseTransferRequest` 和 `DiagnosticsRequest`。
...
当前支持 download 方向的 receiver-paced 单流 open/chunk/ack smoke、活动 download cancel/pause，以及带 source fingerprint 的非 0 offset resume 请求；upload、多流调度、自动断线恢复队列和完整真机传输矩阵仍会继续收口。
...
Mac 端通过 ADB forward 连接这个 endpoint 后，应跑 `m1-smoke` 验证同连接 handshake、heartbeat 和 control-plane RPC，再用 `list-dir` 取一个文件 logical path，并用 `download-cancel` / `download-pause` / `download` 验证传输控制和多 chunk 下载。
