...
真机 smoke 的 `--cleanup-upload-destination` 只自动清理 app-sandbox 和 MediaStore 单段文件名目标。产品协议已有 SAF delete mutation，但 smoke 尚未自动解析 SAF token 并确认递归删除；真机证据仍需显式删除目标并撤销临时 root 授权。
