...
size 精确派生 App Sandbox staging 或 SAF hidden document，和活跃 writer/mutation 共用逻辑路径 claim。
...
- 上述 214 项是 setup fail-closed 收口时的阶段库存；继续加入分页/scan 上限、
  ...
  当前 Android JVM 单元测试库存为 287 项。最大 1000 行页面会保持顺序、只消费
...
- `AndroidPairingCredentialStore`：32 字节 pairing key 由不可导出的 Android Keystore AES-GCM key 包装；pairing ID、设备身份指纹、名称和时间戳全部作为 AAD 认证，密文存入禁备份的私有 SharedPreferences。pairing handler 只在 final confirmation 验证后写入。显式 catalog 扫描逐条完成结构与 GCM 验证后才发布元数据；每项 Base64 值会先按格式推导出的 484 字符上限拒绝，再进入整块解码，超大精确键值仍按结构损坏获得有界清理身份。旧 list API 在目录不完整时仍整体 fail closed。只有严格匹配 `record.<小写 pairing-id hex>` 的后端键可与精确观测密文值、持久逐记录 revision 和损坏分类形成不透明清理身份；payload 名称/ID 不参与操作。save、revoke 与条件清理会在同一次 SharedPreferences commit 中推进 revision/tombstone；缺失 revision 的旧记录按零读取并在首次 mutation 时迁移。删除前会重新扫描并同时原子比较值与 revision，拒绝过期、健康替换、精确 A→B→A 重建或失去健康 key 见证的 GCM 令牌。非严格键、无效 value 类型及畸形/耗尽 revision 不获得清理能力。若没有另一条记录证明当前 wrapping key 可用，GCM tag 失败会按 vault/Keystore 状态未知整体 fail closed；解密或仍存在任何 record 键时绝不自动创建新 key，也不提供删除。
- `AndroidDeviceInfoProvider`：返回设备型号、Android 版本、SDK、数据分区容量、电量和 M1 权限状态。
- `PairingApprovalController` / `PairingAccessibilityPolicy` / `PairedDeviceManager` / `ProductDisplayName` / `ProductReadiness` / `DroidMatchActivity`：进程级 controller 默认关闭；产品 Activity 顶部用可测试的纯状态判定展示“开启 USB → 配对 Mac → 可传输”的明确下一步及可信 Mac/可选文件夹计数，可显式启停安全 USB endpoint，并仅在 paired endpoint 已监听时允许用户打开 120 秒配对窗口。UI 只显示客户端名和六位 SAS，可批准/拒绝 pending attempt、查看按最近使用排序的已配对 Mac、撤销单项信任并立即关闭现有 USB 会话。配对阶段文本作为只在状态真正变化时更新的 polite live region；视觉秒数在独立且从无障碍树隐藏的控件中按秒更新，因此不会反复打断 TalkBack，也不使用 Android 16 已弃用的主动 announcement API。六位 SAS 仅在等待批准时进入无障碍树并逐位朗读，稳定客户端名/配对码不会因 500 ms 轮询重复写入。对端名称在配对批准、可信列表和撤销确认共用同一 UI-only 安全投影：NFC 归一化、折叠空白，移除控制符、Unicode format/bidi 与孤立 surrogate，最多保留 120 个 Unicode code point，真实截断在上限内显示省略号，过滤为空时显示固定 `Mac`；原始名称仍保留在认证 transcript 与 AAD 保护的凭据记录中，配对 ID 继续是撤销身份。SAF 文件夹列表与移除确认也复用该投影并使用本地化空名称 fallback，动作仍由原始 stable root ID/tree grant 定位。普通撤销和异常清理都先经进程级 shutdown 边界封闭 endpoint 准入/socket，并在 1.5 秒内确认全部已准入 RPC worker 退出；超时/失败不删凭据，未排空 endpoint 还会阻止替换 service/endpoint 启动。删除失败时 endpoint 仍已关闭。已配对 Mac 列表暂时不可读时，不会把未知数量误报为零；单记录确定损坏时仍列出已验证 Mac，并明确标记总数不完整。损坏项不显示未认证元数据，只能经二次确认清理；清理还会比较精确观测值、复验损坏分类并在删除后复读整个目录，任何失败都保持目录不可用而不报告零台或成功。对应区域以 polite live region 展示固定说明和显式重试，顶部计数会独立区分“可信 Mac 不可用”“文件夹不可用”及两者同时不可用。照片/视频权限只在用户点击后请求或重选；页面恢复前台时重新读取平台状态，保留总体“全部/受限/关闭”，并分别显示照片和视频的“全部项目/已选项目/关闭”，不会把只拒绝一种媒体的状态留成含糊摘要。SAF 文件夹仍由独立系统 picker 管理；picker 必须返回有效 tree URI 和 read grant，write-only 结果不会调用持久化 API。添加事务在调用前后读取原始 `getPersistedUriPermissions()` 的精确 URI/mode，并额外要求产品 root 可见；失败只释放本次新增的 mode，再复核操作前状态，绝不撤掉原有 mode。移除会从新的原始快照读取该 URI 的全部当前 mode 并一次释放，只有后续快照确认该 URI 已无任何 mode 才成功。空、畸形、重复或不可读快照，以及无法证明回滚/移除完成的结果，均只显示固定脱敏提示；授权列表不可读时，文件夹区与顶部计数都显示暂不可用并提供显式重试，不暴露 tree URI 或平台异常。该事务强化只有本地 JVM/构建证据，不新增真机 UI 声明。
- `SafPickerLaunchGuard`：保持既有系统目录 picker、四项 grant flag 与 request code 不变；当 OEM 缺少 DocumentsUI，或设备/工作资料策略禁止启动时，只返回固定双语脱敏提示并保持产品 Activity 存活。它不会吞掉无关的运行时错误，也不改变选取后的持久授权复核。
...
- `DmFileProvider` 负责 M1 root、进程级 SAF token/authority-epoch cache、catalog 组装/路由与进程级 `ProviderPathCoordinator`；active upload、精确 partial cleanup 与 create/rename/delete 使用同一组非阻塞 logical-path claim。同目标或目标/目录祖先交叉会立即返回稳定 `ALREADY_EXISTS`，provider I/O 不在协调器 monitor 内执行，writer 的 open/write/final/close 任一退场路径都会按 token 释放。SAF claim 同时绑定 authority、stable root、document ID、parent/name 与 token cache 可证明的祖先链；每项操作只读取一次 live roots，并让路由与 claim 共用该不可变快照。链缺失、歧义、成环，或同一 authority 同时存在多个 persisted root 时，会在该操作存活期保守占用整个 authority；即使后续授权快照只暴露一个 root，同 authority 下不同 stable root 的活动 claim 也直接冲突。rename/delete 会推进进程级 authority epoch 并让同 authority 下的旧 document ID token 全部失效；rename 再只为当前 root 重绑 provider 返回的新 ID。listing 只在 epoch 仍一致时批量提交整页 token。`ProviderMediaCatalog`、`ProviderSafCatalog`、`ProviderAppSandboxCatalog` 独立定义 package-private provider 端口和 fail-closed 空实现，具体 Android catalog 不再反向依赖 facade 接口。`ProviderPathRouter` 负责 logical path/target，并在 filesystem canonicalization 前拒绝精确 `.` / `..`、空中间段、NUL 与绝对 relative path；65 行的 `AppSandboxPathResolver` 是 listing、mutation、download 与 upload 共用的唯一 filesystem 准入边界，统一完成 canonical-root 约束并拒绝任何既有符号链接 component；`AndroidAppSandboxCatalog` 继续独占 provider 行为与 staging 生命周期。`ProviderPagePolicy` 独立负责 query-bound opaque page token、分页上限和默认排序；`ProviderBoundedPageSelector` 让 App Sandbox/SAF 在 Java 层最多保留排序前 `offset + pageSize` 个候选，前者还以流式目录扫描避免完整 `File[]`，不发布协议无法安全表示的符号链接；直接寻址链接 component 会 fail closed，App Sandbox 递归删除普通目录时只 unlink 链接节点，绝不遍历链接目标。App Sandbox 下载从已打开描述符 `fstat` 出 size/mtime/device/inode/ctime，后面三类身份只进入不可逆 etag，因此同大小、同 mtime 的原子替换仍会让 resume 指纹失效，而且不会为大文件增加一次全量预哈希。`dm://roots/` 逐次按图片/视频的实时授权生成媒体 `can_read`，相册跟随图片权限；`can_write` 独立，API 26–28 因未声明旧式写权限而不宣称 MediaStore 上传。`AndroidMediaCatalog` 把 limit/offset/sort/search 下推给 MediaStore，并继续唯一持有 resolver/URI、按图片/视频区分的 full/selected/denied 实时权限、selected 模式精确条目可见性、cursor 生命周期、token cache、错误映射、缩略图、传输、pending row 与清理；`MediaStoreCursorReader` 只扫描已打开 cursor，图片保持五列 projection，只有视频根使用含 `MediaStore.Video.DURATION` 的六列 projection；正时长也只有在视频根、MIME 通过 127 字节受限 ASCII 规范化且为 `video/*` 时才进入 additive wire 字段，图片/相册/SAF/App Sandbox/畸形或误分类条目保持零。它同时保留 typed page/album/lookup/metadata 的 null/default、时间单位与 `hasMore` 语义。`AndroidSafCatalog` 负责 persisted tree permission、document query/download、mutation admission 与 parent metadata 验证；`AndroidSafUploadOpener` 独占已授权上传的 final/partial 创建、精确 child 查找、ACK-loss 截断、writer 交接和交接前清理。RESTART 会在删除后复查 exact name 已消失，再要求 create 返回的 document ID 是唯一同名普通文件；delete=false、异常、残留、重复名、null/多行 query、缺失 ID 或 MIME、未知/超出 expected 的旧 partial size 或 provider 自动改名都会在 writer 交接前 fail closed。未验证的返回 identity 会保留而不删除；只有已经 parent/name/ID 对账并交接的 provisional document，才可在后续 open 或 fresh abort 失败时复核后清理。永久 discard 遇到 delete-to-`NOT_FOUND` 竞态时会复查 exact name，仅确认不存在后按幂等成功处理。纯 `SafUploadOpenPolicy` 直接覆盖 fresh/restart/resume 与 partial kind/size 决策；`SafDocumentCursorReader` 仍只用统一六列 projection 把已打开 cursor 解码为 typed item/metadata/child，不持有 resolver、URI、权限、cursor 生命周期或错误映射。`ProviderAuthorizedTransfers` 在 MediaStore/SAF 下载和 SAF 上传每个 chunk 前重查实时授权；Android 14+ selected-media 模式还会重查当前具体条目，避免取消当前条目后旧 FD 继续可读，SAF final bytes 写入后还会在 flush/close/rename 前再次检查；拒绝会先关闭句柄，再由 dispatcher 释放 route 和路径 claim。底层 provider 竞态产生的 `SecurityException` 同样会映射为不泄漏细节的 `ERROR_CODE_PERMISSION_REQUIRED`；若系统撤权先杀死 endpoint，Mac 仍可能只观察到 transport loss。这些权限强化不改变 CRC、offset、传输线格式或严格的 4 chunk / 2 MiB 窗口；`FileEntry.duration_millis` 仅是向后兼容的描述性元数据。
...
- `RenamePathRequest`：App Sandbox 只允许 canonical 同父目录重命名并保持文件/目录 kind；SAF opaque document token 会绑定列出它的真实 parent document，只允许同 root、同 parent 的平台 rename，跨目录、parent provenance 缺失、MediaStore 与只读 provider 都会在 catalog 调用前拒绝。SAF provider 可在 rename 后返回新的 document ID；Android 会在同一 source/destination claim 内 canonicalize 返回 URI，把查询到的唯一 row 与 claimed parent 下唯一 exact-name child 对账，并确认旧 source identity 不再保留。只有名称/kind 精确一致且新 ID 不与既有 token 冲突时才重绑；自动改名、歧义或冲突 identity 会失败并失效旧 cache，既不删除也不重绑未验证结果。目录 create 与上传最终 rename 使用同样的 parent 对账，最终发布还核对 size。
...
- `ThumbnailRequest`：图片/视频使用 MediaStore opaque path；API 29+ 使用 `ContentResolver.loadThumbnail`，API 26–28 使用系统缩略图接口，按最长边 32–512 px 缩放并以最多 512 KiB 的 JPEG 响应返回，不读取并传输完整原文件。音乐封面使用下述独立派生物适配器。
...
- 音乐入口使用实时音频授权：API 33+ 单独请求 `READ_MEDIA_AUDIO`，API 26–32 沿用已有存储读取权限；选中照片/视频不授权音频。`dm://media-audio/` 返回音频列表、可用时长及[有界标题/歌手/专辑](../docs/music-metadata.md)，搜索覆盖文件名和这三项标签，列表发布前再次核对读取权限，API 29+ 可向 `Music/DroidMatch` 新建音频；旧版使用可写 SAF。API 29+ 可通过有界 typed image 派生物提供[音乐封面](../docs/music-artwork.md)，逐次读取及发布前复核音频权限；旧版或无封面时返回不可用。仅本地验证，见[基础音乐](../docs/basic-music.md)。
- `MediaPermissionPolicy` / `MediaPermissionController` / `PermissionStateProvider`：分别负责纯 API 分级、照片/视频细分展示与 fallback 决策、仅由用户操作触发的平台请求/设置跳转，以及不缓存的实时 full/selected/denied 状态；诊断仍只发布粗粒度权限状态。`DiagnosticsReporter` 的计数器有 JVM 并发测试覆盖。
...
- launcher 入口：安装后启动器中显示 DroidMatch 图标，打开 `DroidMatchActivity` 完成安全连接、配对、通知、照片/视频与 SAF 授权管理；它仍不是完整文件管理器。产品 Activity 使用专属 no-ActionBar 深色主题，避免重复标题挤压 API 26 小屏/大字体下的首个安全 USB 操作，同时继续由系统为 API 26–34 保留状态栏与导航栏空间；并排按钮保持等分宽度并共同采用较高标签所需的高度，不裁掉第二行也不形成错位底边。`DroidMatchScreen` 主页面层级中的全部文本和按钮固定使用 simple line breaking 并关闭系统自动连字符，避免 API 26 在窄屏把本地化字符串中没有连字符的普通单词渲染成 `sys- / tem`；超长连续 token 仍可换行，但不会由主页面凭空插入连字符。系统创建的对话框不属于这项策略。定向布局诊断还会滚到页面末尾，确认最终文件夹操作完整处于系统导航区上方。本轮媒体授权/重选只有 JVM、assemble 与 lint 证据，尚无产品 UI 真机归档。
...
同一会话的活跃 `transfer_id` 在上传/下载之间也必须唯一，避免 cancel/pause 命中不确定；成功 cancel 会释放下载 reader 或上传 writer，download pause 只返回最后 ACK 的安全恢复 offset，不会把已发送但尚未确认的窗口数据计入。fresh MediaStore upload 的显式 cancel 只有在精确 pending row 删除一行，或零行删除后再次确认该 URI 已不存在时才成功；异常、仍存在或无法确认会返回稳定错误，并保留同一 transfer 与目标 lease 供再次 cancel。

ADB endpoint 可同时服务多个 loopback 会话，但共享 `DmFileProvider` 会为 active upload、partial cleanup 与 mutation 持有统一的逻辑路径 claim。第二个同目标 open、命中上传目标的 rename/delete，或命中其目录祖先的 mutation 会立即返回 `ERROR_CODE_ALREADY_EXISTS`；无交集路径仍可并行。open 失败、write abort、final commit、成功 cancel、close、mutation 退场和 session teardown 都会释放 claim；未确认 MediaStore row 已清理的显式 cancel 刻意保留 claim。
...
- MediaStore：`dm://media-images/<display-name>`、`dm://media-videos/<display-name>` 和 `dm://media-audio/<display-name>` 是 fresh-only。`ProviderMimeTypes` 以与 Mac Core 对齐的显式扩展名表解析图片/视频/音频 MIME；`ProviderTransfers` 在能力检查、租约和 catalog open 前拒绝未知、无扩展、歧义 `.ts` 或错分类名称，`AndroidMediaCatalog` 在 `insert` 前再次使用同一策略，绝不把任意文件 fallback 成 JPEG/MP4。Android 10+ 会插入 pending row，分别落在 `Pictures/DroidMatch/`、`Movies/DroidMatch/` 和 `Music/DroidMatch/`；final chunk 后只有 `IS_PENDING=0` 恰好更新一个目标 row 才会返回最终成功 ACK，零行更新按提交失败处理并清理未发布 row。非 final close 或 open/write 失败同样会删除插入的 row。显式 cancel 不会把 best-effort delete 当成成功：只有 item URI 删除恰好一行，或删除零行且 item-scoped query 确认不存在，才移除 route 并释放路径 claim；其他结果保留原 writer/transfer/claim，`CancelTransferResponse.ok=false` 只携带固定错误标签，第二次请求可重试同一行。MediaStore upload resume 目前返回 `ERROR_CODE_UNSUPPORTED_CAPABILITY`，可用真机脚本的 `--upload-resume-unsupported-check` 记录这条边界。
- SAF：`dm://saf-<stable-id>/<display-name>` 写入授权 root，`dm://saf-<stable-id>/doc/<directory-token>/<display-name>` 写入已 listing 过的 SAF 目录 token。Android 只接受有写权限且支持 create 的目录；fresh 与 resumable 都先创建 `.droidmatch-upload-` 保留前缀下的 hidden provisional，fresh 非 final close 会删除已验证 provisional，resumable 则保留。整个前缀从 listing 过滤且不可作为用户目标，因此 provider 自动改名后又无法安全删除的残片也不会暴露未完成字节。offset-zero restart 不接受 best-effort 删除：旧 exact-name partial 必须先证明是普通文件且已知大小合法，再在 delete=true 后复查为不存在；新 create 也必须复查为唯一、同 document ID、同精确名称的普通文件，任何无法证明的结果 fail closed。`upload --resume` 要求 partial 至少达到 Mac 最后持久 ACK，超前时通过可写 seekable descriptor 截断到该 offset 后重放；partial 过短或 provider 不支持安全截断时拒绝。final chunk 在再次确认目标名不存在后 rename，并按 claimed parent 对账最终 ID/name/kind/size。SAF 没有可移植的原子 no-replace rename，外部 provider actor 仍可能竞态目标检查，因此这里不宣称跨 provider 无覆盖保证。
