...
> 当前处于 M1 收口期。Mac 已有可构建的 SwiftUI 产品 App 和可挂载验证的本地 DMG；Android 已有配对、连接、信任、媒体权限和 SAF 授权入口，但不是独立文件管理器。Slot C 产品认证/传输已有归档真机证据；Developer ID 签名、公证、USB 插入时延以及更广设备/provider 覆盖仍未完成。
...
- Android 前台连接服务、paired-required loopback ADB endpoint、配对、用户主动触发的照片/视频权限管理、SAF 授权与协议 dispatcher；debug harness 单独保留 nonce-only 证据模式。
