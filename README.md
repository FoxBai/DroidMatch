...
传入 `--handshake-attempts 20 --min-handshake-passes 19 --list-path dm://media-images/` 可记录 handshake 稳定性和首个目录 listing 耗时；传入 `--source-path <dm-path> --resume-check` 时，脚本会先做一次 intentional partial download，再用同一 sidecar/fingerprint 跑 `download --resume`；传入 `--source-path <dm-path> --cancel-check` / `--pause-check` 可记录首块后 `download-cancel` / `download-pause`；传入 `--prepare-app-sandbox-file dm-100mb-zero.bin --resume-check` 会在 app 私有 sandbox 里准备默认 100MiB 测试文件，并自动设置 source/list/min-byte gate。脚本默认会把脱敏后的真机结果写入 `fixtures/m1-runs/`；调试临时运行可加 `--no-result-log`。
