...
- 文件浏览器支持 250ms debounce 的当前目录 provider-side 名称搜索，以及名称、修改时间、大小的升降序切换；过滤和排序均发生在分页前，查询变化会使旧 opaque page token 失效。
...
- 目录浏览，以及 App Sandbox、MediaStore、SAF 的下载和上传能力；MediaStore 图片/视频支持列表/自适应网格、按可见项懒加载的有界缩略图和点击预览。
