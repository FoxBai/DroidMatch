...
传入 `--source-path <dm-path> --resume-check` 时，脚本会先做一次 intentional partial download，再用同一 sidecar/fingerprint 跑 `download --resume`。脚本默认会把脱敏后的真机结果写入 `fixtures/m1-runs/`；调试临时运行可加 `--no-result-log`。
