...
- Android 端已有前台服务、localhost ADB endpoint、framed IO、`ClientHello`/`ServerHello`、`DeviceInfoRequest`、`ListDirRequest` root/media/SAF listing 和 `DiagnosticsRequest` dispatcher、权限状态和诊断骨架。
...
- 当前 Mac harness 已能通过 `m1-smoke` 在同一连接上连续跑 handshake、device info、`dm://roots/` root listing 和 diagnostics；也可以用 `list-dir` 手动验证 `dm://media-images/`、`dm://media-videos/` 和持久化后的 `dm://saf-.../` root。传输 RPC 仍是下一步。
