...
- `dm://saf-.../<name>` 和 `dm://saf-.../doc/<directory-token>/<name>` 目前只支持 fresh upload；`--upload-resume-unsupported-check` 可在已有 writable SAF root 时验证 resume 仍返回 `unsupportedCapability`。脚本不会自动清理 SAF 目标，因为协议还没有 delete/mutation smoke，不能安全地从用户选目录里移除文件。
