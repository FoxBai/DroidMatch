...
- 当前 Mac harness 已能通过 `m1-smoke` 在同一连接上连续跑 handshake、heartbeat、device info、`dm://roots/` root listing 和 diagnostics；也可以用 `list-dir` 手动验证 `dm://media-images/`、`dm://media-videos/`、`dm://app-sandbox/` 和持久化后的 `dm://saf-.../` root，并用 `download` 打开下载传输、逐块校验 CRC32、ACK 后写入本地文件。`download-cancel` / `download-pause` 会在收首块后发送对应 transfer control request 并验证响应；`download --resume` 已能用 sidecar 里的 source fingerprint 请求非 0 offset 恢复；`upload` 已能把本地文件按 receiver-paced chunks 写入 Android `dm://app-sandbox/`。SAF/MediaStore upload、upload resume、自动断线恢复队列和多流调度仍是下一步。
