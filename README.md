...
> **当前阶段：M1 传输与协议验证 + 首个 Mac 产品壳。** 仓库已经具备可运行的 Mac/Android harness、协议实现、真机测试工具，以及可构建的 SwiftUI Mac App：它能通过产品级异步边界发现 ADB 设备，并且不会把硬件 serial 带入界面。Android 启动器已经能显式启停 paired-required loopback endpoint、开放限时配对窗口并处理 SAF 授权。Mac 认证会话装配、文件浏览、传输队列装配、分发签名和完整产品体验仍未完成；Android 入口是安全连接 onboarding/status 页面，不是完整文件管理器。
...
- SwiftUI Mac 产品 target、本地 `.app` 组装脚本和中英文设备发现界面；ADB 进程与 serial 均停留在 Core 边界内。
...
- 首次配对与重连认证的协议、密码学实现和本地测试；Android 产品入口已启用 paired-required 模式，Mac 产品会话与真机认证证据仍待完成。
...
macOS 产品会话、可操作的文件/传输/诊断页面、持久化队列的 app 生命周期装配，以及 AOA 传输仍属于后续工作；当前产品壳不能被描述成完整设备管理器或可分发版本。
