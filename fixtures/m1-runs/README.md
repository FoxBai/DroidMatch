...
For upload smoke runs, prefer disposable filenames and `--cleanup-upload-destination` when targeting `dm://app-sandbox/`, `dm://media-images/`, or `dm://media-videos/`. The script does not clean SAF uploads automatically, so only record SAF upload runs against a disposable user-selected directory or after manually removing the created file.
