...
media references, alias resolution, and external playback are disabled. Audio/video
bytes are not written to a DroidMatch disk cache, diagnostics, or support export.
Native decoder failures produce only fixed localized guidance. Close removes time/status
observers and pending seeks; stale callbacks cannot revive a replaced player. This boundary
...
中文：视频与音乐播放复用已配对认证会话及现有读取/续传能力，每次定位都核对最初源指纹
...
不将音视频写入磁盘缓存或诊断。类型必须匹配路径类别，音乐需显式播放；关闭时移除
监听与跳转。现有证据仅来自本地合成音视频和协议/生命周期检查。
