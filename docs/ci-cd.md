...
no-clobber regular-file and directory-symlink races, validator/link/staging-unlink
failures, and zero-log count. CI cannot replace the physical cable action or the
operator's post-run attestation.
...
普通文件与目录 symlink 的 no-clobber 竞态、validator/link/staging-unlink 失败和零日志计数，
但不能替代真实插线动作与操作者事后确认。
