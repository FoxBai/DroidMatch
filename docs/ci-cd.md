...
pull-request event already validates the corresponding merge candidate. The
dedicated gate ref is the exception that lets an exact candidate SHA earn
protection-eligible checks before a no-PR fast-forward; `main` still receives
the authoritative post-integration regression run. The workflow validates host-buildable gates
...
The macOS job also regenerates every committed Swift protobuf source in a
temporary tree with the `protoc-gen-swift` revision pinned by
`mac/Package.resolved`, then compares the complete file set and bytes. All
repository-owned SwiftPM build, test, product, and device-helper entry points
...
macOS job 还会使用 `mac/Package.resolved` 固定版本的 `protoc-gen-swift`，在临时目录
重新生成全部 Swift protobuf 源码，并逐项比较完整文件集与字节。仓库拥有的 SwiftPM
...
## Generated schema-set migration / Schema 集合迁移

`generate-swift-proto.sh` delegates owned-tree validation to `swift-proto-tree.py`.
When adding/removing a schema changes the output file set, only the canonical
old output may match the complete committed HEAD schema/generated inventory and
exact generated bytes. Staging and final output must match the new shape; symlinks,
mode/ownership/link-count defects and modified predecessors remain rejected.
`test-generate-swift-proto.sh` includes three isolated real-publisher migration
regressions. A transaction whose HEAD/schema basis changes is preserved fail closed.

中文：新增/移除 schema 可从已提交的完整旧生成树迁移；未提交改动不会被覆盖。
恢复期间若基准改变，不保证自动恢复，须保留现场再处理。
...
For R0 owner integration without a PR, use the repository-owned command below.
R1 and R2 changes must use a pull request under `docs/github-governance.md`. The
direct path requires explicit remote-mutation confirmation, an explicit R0
attestation, `DroidMatch-Risk: R0` on every candidate commit, and a clean HEAD
that fast-forwards the freshly fetched live base. The trailer persists the
owner's classification but does not semantically prove that the change is R0.
The command rejects local replace refs, default grafts, and `GIT_GRAFT_FILE`
overrides, rejects any visible `trailer.*` parser configuration, then reads
candidate history with replacement objects disabled and a trusted empty graft
source. It also rejects Git `insteadOf`/`pushInsteadOf` URL rewrites and repeats
that fail-closed check immediately before every Git fetch, remote-ref read, and
push, so a rewrite added during hosted CI cannot redirect the next operation.
If that check blocks cleanup, the exact temporary ref is preserved for manual
inspection. Before any remote push, it runs the local maintainer
contract against that candidate and rejects known test-inventory, wiring, or
takeover-document drift. It then validates Phase A and creates a 128-bit
randomized temporary ref with tag-following and submodule recursion disabled.
All Git network commands use an empty hooks path, controlled pushes additionally
disable verification hooks, and the worktree check disables the optional
filesystem-monitor hook. Repository-local hook code therefore cannot create an
unverified side-channel write around these checks.
Before any write, exactly one fetch URL and one effective push URL must resolve
to the same credential-free GitHub repository identity reported by `gh`; the
resolved endpoints are pinned for the run. An expect-absent lease plus the sole
exact porcelain `new branch` result must prove exclusive creation before the
script claims cleanup ownership. It requires a `push`-event `Spec and Skeleton
Gates` run with the exact branch/SHA, re-fetches
main and protection after that potentially long run, performs a non-forced push,
removes only its own temporary ref under an exact-SHA deletion lease, and waits
for the second, authoritative exact-main run. The local preflight does not
replace hosted admission, and a
manual `workflow_dispatch` run is deliberately rejected as admission evidence.
Each protection read retries a transport/API failure at most three times with a
bounded delay; a successfully read policy that differs from Phase A is rejected
immediately and is never retried into acceptance.
...
If its client result is ambiguous, the script does not discover CI, write main,
or delete the ref even when the candidate is visible: it reports a possible
orphan for manual inspection because same-SHA ownership cannot be inferred.
...
only the exact-SHA leased deletion of the script-owned unique temporary ref; a
changed ref is preserved.

只有 R0 可以使用仓库自带命令无 PR 集成；R1/R2 必须按治理基线使用 PR。命令要求显式
确认远端写入、声明候选满足 R0 边界、每个候选 commit 均带
`DroidMatch-Risk: R0` trailer，并使用干净的可快进 HEAD。trailer 持久记录所有者分级，
但不会在语义上证明改动确属 R0。命令会拒绝本地 replace refs、默认 grafts 及
`GIT_GRAFT_FILE` 覆盖和任何可见的 `trailer.*` 解析配置，并在禁用 replacement
objects、固定空 graft 来源的情况下读取候选历史；同时拒绝 Git
`insteadOf`/`pushInsteadOf` URL 重写，并在每次 Git fetch、远端 ref 读取和 push
紧前 fail closed 地复查，避免托管 CI 等待期间新增的配置改写下一次操作。若复查阻止
清理，脚本会保留精确临时 ref 供人工复核。
随后核验候选的本地维护者契约和 Phase A，以 128-bit 随机值生成临时 ref，关闭标签
跟随与 submodule 递归；全部 Git 网络命令使用空 hooks path，push 额外禁用校验 hook，
工作树检查也禁用可选 fsmonitor hook，避免仓库本地 hook 绕过复查发起旁路写入；并且只有
唯一 fetch URL 与唯一有效 push URL 都解析为 `gh` 报告的同一个无凭据 GitHub 仓库身份
后，才把这两个端点固定到本次运行。只有 expect-absent 租约加唯一、精确的 porcelain
`new branch` 结果证明由本次独占创建后，才取得清理所有权。在该临时 ref 上要求
事件/分支/SHA 都精确一致的 `push` 门禁，
长时间 CI 后重新读取 main 与保护，只做非强制快进，以精确 SHA 删除租约清理自己创建
且未变化的 ref，并等待第二次、
具有发布证据效力的精确 main run；本地预检只会在远端写入前拒绝已知的测试数量、关键
接线和接管文档漂移，不能替代托管准入，手动 `workflow_dispatch` 也明确不能充当准入
证据。每次保护读取
只会对传输/API 失败做最多三次有界重试；一旦成功读取到偏离 Phase A 的策略便立即拒绝，
绝不会通过重试把无效策略变成可接受状态。

只读的 `origin/main` 刷新同样最多有界重试三次，避免成功 push 后的一次网络故障跳过
自有 ref 清理和精确 main CI 观察。候选创建本身绝不重试。临时候选 push 结果有歧义时，
脚本会停止，不查找 CI、不写 main，也不自动删除即使已显示候选的 ref；因为无法从同
SHA 推断创建者，只会报告可能的孤儿 ref 供人工复核。main 快进返回失败后，脚本会先
刷新精确远端 tip：候选已经上线时不重复写入；只有错误明确属于传输故障且 main 仍等于
门禁前基线时才最多尝试三次，并在每次额外写入前重新核验 Phase A；权限/策略拒绝或任何
其他 main tip 都立即停止。Phase A 核验后、重试紧前还会再次刷新并比较 main tip。清理
阶段只允许以精确 SHA 租约重复删除脚本自己创建的唯一临时 ref；已变化的 ref 必须保留：
...
- Protobuf schema and Swift generated-source freshness checks require `protoc`.
  The freshness path additionally builds the lockfile-pinned
  `protoc-gen-swift`; it never accepts `PROTOC_GEN_SWIFT`,
  `SWIFT_PROTOBUF_CHECKOUT`, or alternate-lock environment overrides.
...
- Protobuf schema 与 Swift 生成源码 freshness 检查需要 `protoc`；freshness 路径还会
  构建锁文件固定的 `protoc-gen-swift`，且不接受 `PROTOC_GEN_SWIFT`、
  `SWIFT_PROTOBUF_CHECKOUT` 或替代锁文件的环境覆盖。
...
credential-free private-client environment, private non-raw ADB snapshot pseudonyms,
selected-serial tag/model/API binding, unique insertion
delta, partial-archive and same-revision complete-matrix semantics, AX policy, countdown state machine,
...
rebuild, exact full source and APK hashes, the fixed v2 one-test result, verified
test-package cleanup, and a still-installed product package. CI covers option
rejection, the runner's OEM install/timeout/cleanup matrix, strict field/privacy
validation, allowed directory entries, persistent byte-identical result/`.commit`
pairs, and fixture counts. A disposable repository with a local bare `origin/main`
also executes the formal producer end to end against the real validator/publisher,
including fresh APK hashes, cleanup, stale/dirty refusal, post-run APK drift, and
uncertain-publication propagation; it cannot manufacture a physical pass in the
...
唯一新增设备、部分归档与同一版本显式完整矩阵语义、AX policy、倒计时状态机、artifact metadata、隐私/结构拒绝、普通文件/非 symlink 边界、
...
且产品包仍保持安装。CI 覆盖参数拒绝、OEM 安装/超时/清理失败矩阵、严格字段/隐私校验、
目录允许项、持久且逐字节一致的 result/`.commit` 文件对及 fixture 计数；CI 不能伪造
真机 instrumentation 通过。另有一次性仓库与本地 bare `origin/main` 会通过真实
validator/publisher 端到端执行正式 producer，覆盖全新 APK 哈希、清理、stale/dirty
拒绝、运行后 APK 漂移和发布不确定状态传播；它不会在真实 fixture 目录伪造真机通过。
