...
| `mac-skeleton` | `macos-15` | `tools/check-env.sh --swift`, `tools/run-swift-tests.sh` | Validate Swift harness and Swift Testing availability. |
...
| `mac-skeleton` | `macos-15` | `tools/check-env.sh --swift`, `tools/run-swift-tests.sh` | 验证 Swift harness，并确认 Swift Testing 可用。 |
...
- Mac tests use Swift Testing macros. `tools/run-swift-tests.sh` first tries the
  ...
  Use a full Xcode 16+ toolchain if both paths fail.
