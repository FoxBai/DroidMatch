...
│   │   │   │   ├── RpcEnvelopeValidator.java # Optional payload CRC validation
│   │   │   │   ├── RpcControlHandler.java    # Admitted control payload execution
│   │   │   │   ├── RpcAuthenticationHandler.java # Nonce + paired reconnect
...
│   │           ├── RpcDispatcherEnvelopeIntegrityTest.java
│   │           ├── RpcDispatcherTransferFailureLifecycleTest.java
│   │           ├── RpcTransferFailureTestSupport.java
...
- Uses the Activity-scoped `Theme.DroidMatch` no-ActionBar theme because the screen already owns its descriptive header. This preserves the initial secure-USB action on compact API 26 devices with accessibility font scaling, while the platform still owns the status/navigation safe area
- Gives side-by-side actions equal width and makes both controls adopt the taller label's measured height, so localized or accessibility-scaled labels may occupy a second line without clipping or leaving mismatched button bottoms
- Gives both action rows, the secure-USB enable action, and the final add-folder action stable resource IDs. The opt-in `slot-a-704sh-layout-v2` instrumentation profile requires the exact API/model/viewport/density/font-scale configuration and a multiline English label before it checks initial visibility, both rows' uniform heights, every visible button's measured text/padding height, full scrolling, and final-control visibility above system navigation; other devices skip rather than masquerading as this regression evidence
...
- Regenerate: `bash tools/run-android-gradle.sh :app:generateDebugProto`

**RpcDispatcher / RpcEnvelopeValidator / RpcControlHandler** (`RpcDispatcher.java`, 546 lines;
`RpcEnvelopeValidator.java`, 29 lines;
`RpcControlHandler.java`, 182 lines)
- **Envelope and session-phase request router**
- Requires frame version 1, validates optional bit-0 payload CRC before nested
  parsing, ignores absent/unknown flags, and binds each transfer frame's request
  and stream IDs to the same open route
...
- `RpcControlHandler` parses and executes already-admitted heartbeat, device-info,
  ...
  authentication, transfer, or socket state.

**RpcAuthenticationHandler / RpcPairingHandler / RpcAuthenticationPolicy / RpcSessionState**
...
- `RpcPairingHandler` owns visible first-pairing start/confirm/finalize, approval timeout, identity signature, and final-confirmation-before-persistence
- Both live handlers share one process-local `AuthenticationRateLimiter`; splitting ownership cannot create separate retry budgets
...
- `RpcTransferRegistry` keeps active download/upload handles scoped by session and stream ID; after terminal removal it retains only 16 bounded ID markers per session and consumes at most the four-frame protocol-window tail for each marker
...
**Build:**
```bash
bash tools/run-android-gradle.sh :app:assembleDebug
```

**Test:**
```bash
bash tools/run-android-gradle.sh :app:testDebugUnitTest
```

**Install:**
```bash
bash tools/run-android-gradle.sh :app:installDebug
# or
...
**Lint:**
```bash
bash tools/run-android-gradle.sh :app:lintDebug
```
...
2. Regenerate Java code: `bash tools/run-android-gradle.sh :app:generateDebugProto`
