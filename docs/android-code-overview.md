...
- Gives side-by-side actions equal width but wrap-content height, so localized or accessibility-scaled labels may occupy a second line without being clipped by the former fixed 52 dp row
- Gives the secure-USB enable action a stable resource ID. The opt-in `slot-a-704sh-layout-v1` instrumentation profile requires the exact API/model/viewport/density/font-scale configuration and a multiline English label before it checks initial visibility and every visible button's measured text/padding height; other devices skip rather than masquerading as this regression evidence
