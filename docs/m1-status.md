...
The [September integration record](m1-integration-2026-09.md) lists the accumulated
recovery, cancellation, permission, and evidence-tool changes plus their review
boundaries. It does not add physical-device evidence.

For remaining work beyond M1 acceptance, use the [Project Backlog](project-backlog.md).
...
- [APK installation](apk-installation.md) adds paired-only prepare/list/cancel and
  ...
  only Android's foreground action submits to the ordinary system installer.
  Durable commit intent, verified cleanup and explicit unknown-outcome recovery
...
- [Basic Music](basic-music.md) adds a fourth Media section with bounded audio
  ...
  the existing queue. Android has an explicit live Music permission control:
  independent audio access on API 33+, shared legacy read on API 26–32, and
  app-owned fresh-only Music inserts on API 29+. Old peers without the root show
  unavailable. Local provider/permission/queue/lifecycle and synthetic native UI
  coverage exists. Music now opens an idle preview and starts native bounded
  playback only on Play, with pause/seek/replay and explicit close cleanup.
  Paired range/lifecycle tests and generated WAV/M4A playback provide local
  evidence; native format failures retain download. Physical music/codec behavior,
  artwork compatibility and artist/album group views remain open. [Music metadata](music-metadata.md) adds bounded title/artist/album display and search with filename fallback and unchanged transfer names. [Bounded Music artwork](music-artwork.md) now reuses row/preview requests with live audio access, pre-decode budgets and native placeholders; API 29+ provider output remains physically unverified. No existing M1 gate is newly passed.

- Supported MediaStore video previews now provide native playback, pause, and
  seek. MP4/QuickTime/M4V/3GPP containers use the existing paired download
  protocol through serial ranges capped at 1 MiB, preserving the original
  fingerprint/size, CRC/offset validation, provider permissions, and wire window.
  The preview context rejects late opens/bytes after close, replacement, refresh,
  navigation, or authorization loss. AVFoundation receives only a random asset
  URL; external references, alias resolution, and external playback are disabled,
  and DroidMatch writes no video cache. Local paired-TCP, lifecycle, and synthetic
  native decode/play/pause/seek checks cover this enhancement. Physical playback,
  OEM/provider behavior, and the remaining M1 gates are still unverified.
...
- SwiftUI `DroidMatch` product target with English/Chinese device dashboard, independent Media sidebar, canonical-path localization for built-in provider roots, readable navigation titles instead of opaque paths, async ADB discovery, process-local opaque device IDs, stale-snapshot disclosure, generated native icon, and a verified ad-hoc local `.app` bundle. Files hides the Images, Image Albums, Videos, and Music roots; Media is the sole product media entry, where Images, Albums, Videos, and Music retain separate browser state while reusing the authenticated paging/search/sort/grid/preview/transfer surface. The native Help menu now opens a local bilingual SwiftUI guide instead of the system's missing-Help-Book alert; its checked source has no network URL, device-session, or Keychain dependency. A nonce-only debug endpoint is surfaced as `secureEndpointRequired` with an actionable Secure USB instruction rather than a generic transport failure.
...
- Protobuf-free product directory domain types plus paged `AsyncRpcControlClient` listing and embedded-error/row/token validation; optional provider MIME is canonicalized to a restricted lowercase ASCII value of at most 127 bytes, while malformed metadata degrades to nil without changing row identity, capability, or authorization. `DirectoryBrowserPresentationTypes` keeps UI-only bidi/control-safe names plus independent browse/upload projections without changing remote identity, `DirectoryBrowserPolicy` keeps direct-child/mutation/media/error decisions pure, and the MainActor `DirectoryBrowserModel` alone owns clients/tasks/generations, atomic refresh, retryable load-more, stale-generation rejection, cross-page deduplication, and sanitized published state. Create/rename/single-delete/batch-delete displays capture one model-issued opaque context; navigation, query load/refresh/load-more, authorization invalidation, reset, and every applied page rotate it. Rename/delete additionally require complete captured item equality and batch delete requires the exact captured set, so stale or exact A→B→A confirmations fail before the runner/RPC with fixed privacy-bounded guidance, while already admitted work still drains across navigation. A pure `DirectoryBrowserThumbnailState` owns only thumbnail generation/FIFO/active-key/failure/cache transitions; it retains draining old-generation requests against the four-request limit while rejecting their publication, and bounds cached images to 64 entries/8 MiB without owning a client, Task, permission decision, or Published value. Navigation cancels only the old listing and queued old-generation row thumbnails, not an admitted mutation; same-path completion refreshes the current search/sort query, while another path suppresses the stale result/error. A global category change or the final visible browser surface leaving clears queued work, preview, and cache without losing listing/query/navigation; one stale surface departure cannot invalidate another visible window's derivative generation or preview. The 512-pixel preview is outside the thumbnail queue and may be the fifth control request. Each visible browser surface and preview sheet holds a model-issued, process-local opaque context that cannot be derived from its item or path; only the current context can consume or clear preview state. A stale window dismissal, navigation, refresh, or authorization invalidation therefore settles to fixed unavailable guidance instead of showing another window's image or an idle loading spinner. One browser model still admits at most one real preview request at a time, and admitted work drains. Listing pagination uses separate preview/thumbnail validity, so load-more cannot strand the current preview in a loading state. Unreadable roots no longer trigger a list request, while an independently writable root retains a direct upload action. Native picker and Finder drop batches share one tested admission policy: 1–100 ordered non-symlink regular files, normalized-name uniqueness, and exact destination/media filename validation.
- Video/audio listings carry additive MediaStore duration only for a positive
  row with matching `video/*` / `audio/*` MIME. Android leaves every image, album, SAF, App Sandbox,
  unknown, and misclassified value at zero; Mac independently fails those
  values closed to nil and renders accepted durations in list, grid, and preview
  surfaces as `m:ss` or `h:mm:ss`. Duration alone grants no playback capability
  and adds no physical-device evidence.
- Create, rename, single-delete, and batch-delete failures use operation-specific fixed guidance. The model publishes the operation and bounded failure category atomically, so every window observing the process-shared browser renders the authoritative operation instead of reinterpreting it through local sheet state. A synchronous create/rename admission rejection remains visible in its edit sheet, while an admitted asynchronous failure is shown by the browser; neither path exposes an item name, logical path, or raw exception. 中文：创建、重命名、单删与批删按操作使用固定脱敏说明；模型会原子发布操作类型与有界失败类别，因此观察进程级共享浏览器的每个窗口都使用权威操作，不会通过本地 sheet 状态误解失败。同步准入拒绝留在编辑 sheet 内，已准入后的异步失败回到浏览器提示，两者都不携带条目名、逻辑路径或原始异常。
...
- Mac notification Settings reconcile the stored opt-in with live macOS authorization on appearance, foreground return, and an explicit enable request. Denial or later revocation keeps/resets the preference off instead of presenting an undeliverable enabled state; an explicit rejection gets fixed bilingual guidance, and generation checks reject late permission callbacks. A completion event must observe the opt-in enabled, while final enqueue compares its event generation with the current MainActor-owned preference generation and live authorization, so disabling notifications cannot leave one late alert and an off/on cycle cannot revive an older candidate. Three pure policy tests cover passive revocation, explicit request results, and continuous opt-in/live-permission delivery; this is local evidence and does not request a real system permission during tests.
...
- Running fresh-only MediaStore product cancellation is now transactional at the existing active upload boundary: queue admission becomes non-terminal `cleaning`, sends `CancelTransferRequest` with the exact transfer ID, and settles `cancelled` only after remote success. Typed failure retains the same session/route for user retry with no further chunk refill. A final ACK observed first makes the cancel lose, while normal post-ACK source revalidation remains authoritative: a stable source completes and a changed source retains its upload-source failure. A pre-open failure proves no remote row exists, while transport loss or session teardown before confirmation persists cleanup-unverified `interrupted` with no fresh auto-replay. This has local Swift/TCP evidence only and adds no physical-device claim.
...
- Product launcher entry (`DroidMatchActivity`) with a tested next-step readiness summary, paired-required endpoint controls, pairing approval, secret-free paired-Mac list/revoke, notification permission, user-triggered photo/video authorization or reselection with a live full/limited/denied summary plus separate image/video all-items/selected-items/off rows, and SAF authorization list/add/revoke. The aggregate and both detail rows derive from the same two live permission reads, so a one-media-type denial is no longer hidden behind only “limited.” Peer-controlled Mac names now cross one UI-only safe-display projection before pairing approval, trusted-list, or revoke-confirmation rendering: it NFC-normalizes, collapses whitespace, removes control/Unicode-format/surrogate code points, and uses fixed `Mac` when nothing visible remains. The authenticated raw name stays in the transcript and encrypted credential metadata, while pairing ID remains the revoke identity. SAF add/release now uses raw `getPersistedUriPermissions()` exact URI/mode snapshots: non-tree or write-only picker results are never taken, failed product-root confirmation releases only modes newly added by this attempt and preserves prior modes, and remove releases every freshly observed mode rather than trusting cached UI capability. Success requires the requested add modes plus product root, or exact URI absence after removal; snapshot exceptions, malformed/duplicate state, residual modes, and unprovable rollback produce fixed privacy-bounded guidance. An unreadable list marks both the list and top-level folder count unavailable and exposes an explicit retry. Trust revocation closes active USB sessions, while diagnostics harness naming remains confined to debug source. A successful paired reconnect monotonically updates the credential's encrypted last-used timestamp; persistence failure is diagnostic-only and does not invalidate a correct proof. Media root `can_read` now follows live image/video access independently from `can_write`; this product permission flow and the SAF grant transaction hardening have local JVM/wiring/build evidence but no archived physical UI pass.
...
- Android pairing-vault inventory now verifies every record's full structure and AES-GCM tag before exposing metadata. The format-derived 484-character encoded ceiling is checked before whole-value Base64 allocation; an oversized exact-key value remains removable structural damage. A definitely malformed record no longer hides healthy records from the same explicit catalog scan; legacy list APIs still fail closed when it is incomplete. The launcher labels the catalog incomplete, hides all unverified payload fields, and offers only explicit-confirmation cleanup bound to an exact lowercase backend pairing-ID key plus its exact observed encrypted value, persistent per-record revision, and damage class. Save, revoke, and conditional cleanup advance that revision/tombstone in the same SharedPreferences commit; a legacy record without one reads as zero and migrates on its first mutation. Cleanup closes endpoint admission/sockets and waits up to 1.5 seconds for all admitted RPC workers to exit before a fresh damage rescan and atomic value-plus-revision compare-and-remove; timeout/failure preserves the record and blocks replacement endpoint startup until a later retry drains the old workers. It then requires a fresh catalog reread before reporting success. A stale token cannot delete a healthy, changed, or exact-value A→B→A replacement, and a GCM token expires if its healthy-key witness disappears. Non-exact keys, invalid backend value types, or malformed/exhausted revisions mark the catalog incomplete but receive no cleanup capability. A GCM failure without another successfully authenticated record remains a vault-wide/Keystore-unknown failure with no cleanup action, preventing missing, replaced, invalidated, or transient key state from being treated as deletable corruption. Authentication lookup remains fail-closed. This has focused local JVM/resource/assemble evidence only and no new physical Keystore or UI claim.
...
- Trusted-device display and credential selection now use separate Keychain
  ...
  only the fingerprint-matched current credential. Malformed current-format
  ...
  item was the intended target. A successful reconnect does
  ...
  secret reads for first pairing, one shared legacy authentication context, and
...
- `tools/push-main-with-gates.sh`: explicit-confirmation, R0-attested no-PR owner integration that requires every candidate commit to persist exactly `DroidMatch-Risk: R0`, rejects local trailer-parser aliases and Git URL rewrites, requires a clean fast-forward HEAD, and rejects known maintainer-contract/inventory drift before any remote push. It validates Phase A before and after candidate CI and runs the exact SHA through a randomized protection-eligible temporary `push` ref only after an expect-absent lease plus one exact porcelain creation record prove ownership. Exactly one fetch endpoint and one effective push endpoint are pinned to the same credential-free GitHub repository identity. Every push disables tag following and submodule recursion; main is never force-pushed, and temporary cleanup uses an exact-SHA lease that preserves changed refs. An ambiguous candidate-push result stops before CI discovery or main mutation and is not auto-cleaned. A failed main-push result is resolved against the exact remote tip: an already-live candidate continues without a duplicate write, while only an explicit transport failure with main still at the pre-gate base receives at most three attempts, with Phase A revalidated and the exact main tip refreshed immediately before every extra write. The flag and trailers persist the maintainer's R0 assertion but do not semantically classify the change; R1/R2 use a PR. The command returns success only after the exact `main push` run also passes and protection remains intact. The local preflight does not replace hosted admission; mock and real-bare-Git suites cover missing/wrong trailers, parser aliases, chained URL rewrites, multiple/mismatched endpoints, no-follow behavior, same/different-SHA ref collisions, ambiguous candidate results, leased cleanup, bounded main transport recovery, retry-time protection/tip drift, and remote-mutation ordering
...
- Android stable identity signing, its default-closed 120-second visible pairing
  window backed by Android elapsed-realtime milliseconds that include device
  deep sleep, start/confirm/finalize dispatcher, async Mac client, and
  provisional Keychain rollback are implemented with JVM and loopback
  end-to-end tests.
...
- Mac and Android both expose secret-free trust management. Mac revocation waits for active-session teardown before deleting the Keychain record; a failed/false deletion retains the trusted row, marks the snapshot unavailable, and presents only fixed privacy-bounded guidance. While a display-only Keychain list remains outstanding, the Mac model and revoke button fail closed before session teardown, mutation state, or data-source deletion; admission reopens only after that request returns. Confirmation synchronously closes device-card Connect/Reconnect, then an admitted revoke atomically reserves model mutation before awaiting teardown, so refresh, another revoke, and connection entry remain closed until the post-teardown Keychain deletion succeeds or fails and releases the reservation. Card connection and panel retry actions re-check that admission when they execute, rejecting work queued before the controls updated; this prevents credential reload from creating a replacement authenticated session inside the revocation window. Teardown failure or cancellation releases the reservation without deleting. This guard does not cancel or bypass a Security.framework call already inside the OS. Android revocation closes active USB sessions. Slot C ordinary-App first pairing, paired reconnect, sandboxed product authentication, and attended real Android Keystore behavior are archived.
...
- Concurrency: both the stable M1 probe and product async core have bounded two-stream paths
  ...
  - The shared Android provider uses one non-blocking process-level path coordinator for uploads, resumable-partial cleanup, and create/rename/delete. Same-target and directory-ancestor overlap returns `alreadyExists`; unrelated paths remain independent. SAF uses authority/document/parent-name plus cache-proven ancestry, with a temporary whole-namespace fallback when lineage cannot be proved or multiple persisted roots share one authority. Successful rename/delete requests invalidate the old provider identity across those roots, while an exact rename rebinds only the current root; parent-token resolution and the bounded authority-scoped cache epoch are captured atomically. Exact directory creation advances that epoch and invalidates a stale same-parent/name child across overlapping roots. Upload open/final/close/discard uses the same targeted invalidation while preserving the parent token required by nested retry/resume. An uncertain ordinary mutation still invalidates provider-scoped tokens. JVM tests cover commit, abort, cancel, failed open, mutation exit, session teardown, a latch-controlled upload-open/delete race, complete/incomplete SAF lineage, overlapping-root exclusion, cross-root rename/delete and targeted child invalidation, both mutation/listing linearization orders, stale-listing rejection, nested-token preservation, exact create/upload epoch advancement, and idempotent discard after a verified delete-to-missing race.
  - Every SAF upload, including fresh upload, stages bytes under the reserved hidden `.droidmatch-upload-` namespace; listings and direct destinations reject that whole prefix, including provider auto-renamed remnants. Offset-zero restart first proves an old exact-name item is an ordinary file with a known acceptable size, then requires delete=true plus an absence re-query and a unique exact-name ordinary file whose ID matches the create result. Null/multi-row queries, incomplete IDs or MIME metadata, invalid old kind/size, delete false/exception, residual/duplicate names, auto-rename, and wrong created kind fail before writer handoff. Directory creation, ordinary rename, and final publication canonicalize the returned URI and reconcile the returned ID/name/kind with the unique claimed-parent child; publication also checks the final size. Unverified returned identities are never deleted or rebound. Only an already verified provisional document may be removed after a later open/abort failure. SAF has no portable atomic no-replace rename, so an external provider actor can still race the destination check; this remains a device/provider validation risk rather than a no-clobber guarantee.
  ...
  - Visual product downloads carry an explicit `mustBeAbsent` publication policy from single/batch panel planning through both product data sources, schema-v3 queue persistence/resume, and atomic-writer acquisition/commit. A same-name target created after preflight or during transfer is preserved; final absent-target publication still uses an exclusive rename. Schema-v1/v2 manifests without the field restore fail closed to no-clobber, product restoration rejects replacement policy, and explicit `replaceExisting` remains a Core compatibility/harness option only. Four offline regressions cover policy persistence plus sentinel preservation before writer initialization and commit; no physical-device claim is added.
  ...
  - Resumable upload creation now has a strict double write-ahead boundary: the
    v2 sidecar and schema-v3 queue both persist the exact destination/transfer/
  ...
  - The scheduler's fresh MediaStore active-cancel path does not cancel the owning Swift task or close the session as a cleanup substitute. It holds the row in `cleaning`, blocks refill, and reuses one controller/handle across typed remote failures; remote cancel success, final ACK plus stable post-ACK validation, and session loss reconcile respectively to `cancelled`, `completed`, and persistent cleanup-unverified `interrupted`. A post-ACK source-identity failure remains the original bounded upload-source failure rather than being relabeled as cleanup uncertainty.
...
The owner confirmed that physical testing is temporarily unavailable.
The following gates remain open while offline product work continues; no passing
fixture or M1 completion is inferred from that scheduling decision.
...
6. **Large directory stress tests:**
   ...
   - ✅ Local Java memory shape: app-sandbox streams directory entries and both
     ...
     MediaStore pushes limit/offset/sort to `ContentResolver` and uses `_ID` as
     the deterministic secondary key for every non-ID offset sort
...
As of 2026-09-22, `fixtures/m1-runs/` contains:
- 90 test result logs
