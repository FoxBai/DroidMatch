...
- The current source inventory is 503 Swift tests and 286 Android JVM tests. The Android foreground notification now distinguishes endpoint preparation from a successfully bound listener; a current-generation failure or unexpected exit closes pairing, removes the notification, and stops the idle service while retaining a retryable `FAILED` UI state. Three pure lifecycle tests cover ready publication, current failure, and stale-generation rejection; they do not constitute foreground-service or attended-device evidence. Two Presentation regressions show that dependency-owned cancellation during connect or pairing before or after approval exits the busy phase through the stable `connectionUnavailable` state, while actual UI-task cancellation remains silent; they do not add transport, credential, or device evidence. Three preview-context regressions prove that shared browser windows cannot cross-publish, cross-dismiss, or invalidate the current preview when a stale surface departs, same-path refresh cannot reuse an old context, navigation and authorization invalidation settle to fixed unavailable state, and admitted requests still drain one at a time. Three pure search-admission regressions show that the file/media browser retains only the latest debounced edit while listing, mutation, or transfer submission is busy, submits it when the shared surface becomes available, and rejects stale edits after navigation or sort changes. Existing session-lifetime coverage additionally proves globally latest-token admission, stale-window cancellation isolation, and context invalidation without adding a test. The opaque token contains no query, path, or user input; none of these tests additionally proves globally latest-token admission, stale-window cancellation isolation, and context invalidation without adding a test. The opaque token contains no query, path, or user input; this adds no provider or device evidence. Android's pairing countdown remains visual in a separate accessibility-hidden view; a stage-only polite live region changes only for meaningful closed/waiting/approval/approved/rejected transitions, without using Android 16's deprecated explicit announcement API. The pending SAS is exposed as six separately spoken ASCII digits, and unchanged 500 ms stage/client/code writes are suppressed. Three picker-launch guard tests cover success, missing/policy-blocked picker failures, and propagation of unrelated runtime failures; six SAF grant transaction tests replace the former single confirmation test and cover pre-take rejection, exact mode reconciliation, incremental rollback, residual revoke state, and fail-closed snapshots. This is offline evidence, not an attended-device accessibility or picker-UI claim.
