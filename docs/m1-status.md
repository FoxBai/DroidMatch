...
- Mac discovery now enriches device cards with a display-only retail name while retaining raw model/product as secondary technical context. SHARP 704SH resolves offline to `シンプルスマホ４`; another cache miss schedules an ephemeral no-cookie/no-redirect streamed request to Google's one exact public full-device catalog URL. A dedicated catalog-loader actor enforces 8 MiB/UTF-16LE/header/row/field limits and builds a bounded index, while the resolver actor caps pending tuples at 64. Matching/cache identity uses complete accepted 512-scalar identifiers, not the 120-scalar UI projection, and only an unambiguous raw name is projected. No serial or per-device search term leaves Core; at most 512 safe resolved names persist under SHA-256 tuple keys, and any failure falls back to the existing safe technical label. Five direct resolver/discovery regressions bring the current Swift inventory to 470. This is local automated evidence only and does not claim a new current-tip 704SH physical pass.
...
As of 2026-07-18, `fixtures/m1-runs/` contains:
- 89 test result logs
