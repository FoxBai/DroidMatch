...
- Video listings now carry additive MediaStore duration only for a positive
  video-root `video/*` row. Android leaves every image, album, SAF, App Sandbox,
  ...
  surfaces as `m:ss` or `h:mm:ss`. This is local protocol/UI evidence only and
  does not claim playback, range streaming, or new physical-device coverage.
