...
- Mac notification Settings reconcile the stored opt-in with live macOS authorization on appearance, foreground return, and an explicit enable request. Denial or later revocation keeps/resets the preference off instead of presenting an undeliverable enabled state; an explicit rejection gets fixed bilingual guidance, generation checks reject late permission callbacks, and delivery still rechecks authorization immediately before enqueueing a notification. Two pure policy tests cover passive revocation and explicit request results; this is local evidence and does not request a real system permission during tests.
