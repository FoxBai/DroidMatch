...
- `tools/push-main-with-gates.sh`: explicit-confirmation, no-PR owner integration that requires a clean fast-forward HEAD, rejects known maintainer-contract/inventory drift locally before any remote push, validates Phase A before and after candidate CI, runs the exact SHA through a unique protection-eligible temporary `push` ref, rejects a changing main tip or wrong event/run identity, never force-pushes, and first resolves a failed main-push result against the exact remote tip; an already-live candidate continues without a duplicate write, while only an explicit transport failure with main still at the pre-gate base receives at most three attempts, with Phase A revalidated and the exact main tip refreshed again immediately before every extra write. It cleans its owned ref and returns success only after the exact `main push` run also passes and protection remains intact; the local preflight does not replace hosted admission, and the offline suite covers preflight rejection, bounded transport recovery, ambiguous-result resolution, retry-time protection/tip drift, remote-mutation ordering, and every fail-closed boundary
