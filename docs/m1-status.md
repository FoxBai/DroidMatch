...
- Transport-loss retry: configurable multi-attempt recovery queue now implemented
  ...
  - Core has an opt-in on-disk queue manifest and restoration factory. The Mac App supplies a private per-device Application Support location, restores behind an execution latch, and verifies every non-terminal local endpoint against transactionally persisted App-owned bookmarks before activation. Corrupt/unreadable recovery state, or a bookmark archive empty or incomplete for those restored targets, stays `writeFailed` without replay; one explicit retry reloads bookmarks, reloads the manifest without execution, verifies the resulting target coverage, then unlocks the scheduler. Session teardown still suspends the queue before releasing transport.
