...
- Concurrency: both the stable M1 probe and product async core have bounded two-stream paths
  ...
  - The scheduler's fresh MediaStore active-cancel path does not cancel the owning Swift task or close the session as a cleanup substitute. It holds the row in `cleaning`, blocks refill, and reuses one controller/handle across typed remote failures; remote cancel success, final ACK, and session loss reconcile respectively to `cancelled`, `completed`, and persistent cleanup-unverified `interrupted`.
