...
- The current source inventory is 510 Swift tests and 283 Android JVM tests. Three preview-context regressions prove that shared browser windows cannot cross-publish, cross-dismiss, or invalidate the current preview when a stale surface departs, same-path refresh cannot reuse an old context, navigation and authorization invalidation settle to fixed unavailable state, and admitted requests still drain one at a time. Three pure search-admission regressions show that the file/media browser retains only the latest debounced edit while listing, mutation, or transfer submission is busy, submits it when the shared surface becomes available, and rejects stale edits after navigation or sort changes; none of these tests add provider or device evidence. Android's pairing countdown remains visual in a separate accessibility-hidden view; a stage-only polite live region changes only for meaningful closed/waiting/approval/approved/rejected transitions, without using Android 16's deprecated explicit announcement API. The pending SAS is exposed as six separately spoken ASCII digits, and unchanged 500 ms stage/client/code writes are suppressed. This is offline evidence, not an attended-device accessibility claim.
