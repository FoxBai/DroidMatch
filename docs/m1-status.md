...
- The current source inventory is 504 Swift tests and 283 Android JVM tests. Three pure search-admission regressions show that the file/media browser retains only the latest debounced edit while listing, mutation, or transfer submission is busy, submits it when the shared surface becomes available, and rejects stale edits after navigation or sort changes; they do not add provider or device evidence. Android's pairing countdown remains visual in a separate accessibility-hidden view; a stage-only polite live region changes only for meaningful closed/waiting/approval/approved/rejected transitions, without using Android 16's deprecated explicit announcement API. The pending SAS is exposed as six separately spoken ASCII digits, and unchanged 500 ms stage/client/code writes are suppressed. This is offline evidence, not an attended-device accessibility claim.
