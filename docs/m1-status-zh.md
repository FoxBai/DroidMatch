...
- 传输丢失重试：现已通过 `RecoveryPolicy` 实现可配置的多尝试恢复队列
  ...
  - Core 已有可选磁盘队列 manifest 与恢复 factory；Mac App 已提供私有的按设备 Application Support 存储位置，通过 App 自有 bookmark 重新取得 security-scoped 访问，并公开持久化失败/中断状态和显式的 Core 权威持久化重试，在会话拆除前暂停队列。
