...
- 当前源码库存为 503 项 Swift 测试与 286 项 Android JVM 测试。两项 Presentation 回归证明连接或配对批准前后由依赖自身产生的取消会退出忙碌阶段并进入稳定的 `connectionUnavailable` 状态，而真正的 UI task 取消继续保持静默；它们不新增传输、凭据或真机证据。Android 前台通知现会区分 endpoint 准备中与当前 listener 已成功绑定；当前 generation 失败或意外结束时会关闭配对、移除通知并停止空闲 service，同时保留可显式重试的 `FAILED` 界面状态。三项纯 lifecycle 测试覆盖就绪发布、当前失败和旧 generation 拒绝；它们不构成前台 service 或真机证据。三项 preview-context 回归证明共享浏览模型的窗口不会跨 context 发布、清除预览，也不会让旧 surface 离场失效当前预览、同 path refresh 不能复用旧 context、导航与授权失效会进入固定不可用状态，且已准入请求仍按单飞顺序排空。三项纯搜索准入回归证明文件/媒体浏览器在 listing、mutation 或 transfer submission 忙碌时只保留最后一次 debounce 输入，界面恢复可操作后立即提交，并在导航或排序改变后拒绝旧输入。既有会话生命周期测试另证明全局最新 token 准入、旧窗口取消隔离及上下文失效而不增加测试项；不透明 token 不含 query、path 或用户输入。这些测试都不新增 provider 或真机证据。Android 配对倒计时仍在独立且从无障碍树隐藏的控件中正常显示；阶段专用 polite live region 只在关闭、等待、待批准、已批准、已拒绝等真实变化时更新，不使用 Android 16 已弃用的主动 announcement API。等待批准时 SAS 作为六个独立 ASCII 数字朗读，500 ms 轮询中的未变化阶段/客户端/配对码写入会被抑制。三项 picker 启动保护测试覆盖正常启动、选择器缺失/受策略禁止及无关运行时错误继续传播；六项 SAF 授权事务测试替换原单项确认测试，覆盖 take 前拒绝、精确 mode 对账、增量回滚、撤销残留和快照 fail-closed。这些只属于离线证据，不新增真机无障碍或 picker UI、Developer ID、公证结果。
