...
**工具：**
- `tools/check-source-size.py`：新增手写生产源码 1000 行上限，以及三个已记录存量巨石文件只降不升的债务上限
...
- **结构性债务仍存在：** Android transfer RPC 路由与 ACK 有界 stream 状态已从 `RpcDispatcher` 抽离，使其从 2293 行降至 1363 行；972 行 provider facade 已不再需要例外，但 pairing/authentication 仍留在 dispatcher，三个生产巨石文件仍受显式“不得增长”上限约束；见[结构性债务基线](technical-debt.md)
