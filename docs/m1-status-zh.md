...
- **结构性债务仍存在：** 全部手写生产文件均已回到默认 1000 行预算，且没有例外；Mac harness dispatcher 为 786 行，transfer 命令已拆成独立的 677 行 Core consumer。同步 CLI 网络与所有权集中仍待继续治理；见[结构性债务基线](technical-debt.md)
