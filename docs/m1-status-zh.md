...
**工具：**
- `tools/check-source-size.py`：新增手写生产源码 1000 行上限，以及四个已记录存量巨石文件只降不升的债务上限
...
- **结构性债务仍存在：** scheduler 公共契约、RPC one-shot 原语、Android provider transfer-I/O 状态和 app-sandbox catalog 已抽离，但仍有四个生产巨石文件受显式“不得增长”上限约束；见[结构性债务基线](technical-debt.md)
