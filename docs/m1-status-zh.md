...
- 原 774 行目录浏览 MainActor 现以 708 行继续持有 Published/listing/导航状态、派生 Task、预览、权限判断、mutation context 轮换及按 path 应用 mutation 结果。46 行预览状态边界持有进程内 opaque context 及固定 loading/ready/unavailable/invalidated 展示状态。132 行纯缩略图状态独占 generation/FIFO/active-key/失败/缓存 transition，并让排空中的旧请求继续计入四项上限；三项直接测试覆盖旧 generation 并发、去重/可见性/失败准入及缓存双上限。157 行 MainActor runner 另独占活跃远端 mutation Task 与操作身份且不持有展示或刷新策略。三项 focused preview-context 回归覆盖跨窗口发布/旧 dismiss、同 path refresh、导航、授权失效、单请求准入和 drain-safe 完成；三项 focused mutation-context 回归覆盖目录替换、完整条目替换与精确 A→B→A 回摆且零 RPC；已准入后导航回归继续通过。本项只有本地证据，不新增真机声明。
