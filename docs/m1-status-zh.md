...
- **结构性债务仍存在：** scheduler 公共契约、RPC one-shot 原语、Android provider transfer-I/O 状态、三个平台 catalog 和 provider 路径路由均已抽离；provider facade 现为 972 行且不再需要例外，但另外三个生产巨石文件仍受显式“不得增长”上限约束；见[结构性债务基线](technical-debt.md)
