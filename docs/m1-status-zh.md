...
- **结构性债务仍存在：** 全部手写生产文件均已回到默认 1000 行预算，且没有例外；基线 `m1-smoke` 与普通 `list-dir` 已使用产品 async transport，仍待治理的是 handshake-only、expected-error listing、transfer probes 与所有权集中；见[结构性债务基线](technical-debt.md)
