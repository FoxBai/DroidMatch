...
- **应用管理：** [主动共享的可启动应用信息](application-library.md)、搜索排序、分页及
  [单 APK 系统确认安装](apk-installation.md)及 [单包/分包导出](apk-export.md)已本地实现；真机验证仍待做。
- **真机证据：** 普通与 sandbox Slot C 产品认证、浏览、传输、恢复，以及需要人工参与的
  断线/续传结果已经归档。
- **开放的 ADB M1 阻塞项：** Slot A 仍缺 current-candidate release 下载/上传吞吐证据；
  Slot A/C/D 仍缺产品 USB 插入到可见设备不超过五秒的人工证据。
- **分发：** 本地 ad-hoc App 与 sandbox DMG 已有验证，Developer ID 签名、公证和发布
  自动化仍按当前决定暂缓。

判断是否通过里程碑时，优先阅读[M1 退出标准进度](#m1-退出标准进度)、
[即时下一步](#即时下一步)和[测试结果摘要](#测试结果摘要)。下方详细实现库存是参考材料，
不是最短的状态阅读路径。

[九月集成记录](m1-integration-2026-09.md)列出本次恢复、取消、权限与证据工具修复及审查边界；
这些改进不新增真机证据。

全项目后续功能、真机待办与发行边界见[全项目待办](project-backlog.md)。

## 当前实现状态

### ✅ 已完成功能

- [APK 导出](apk-export.md)使用仅配对能力 11 和独立手机授权。普通应用导出原始 APK，分包应用导出含全部已安装分包与 SHA-256 清单的 ZIP64；会话快照拒绝撤权、更新和不完整分包。原生目录授权保持到传输退出，完整输出通过原子无覆盖发布，取消仅清理身份已确认的未发布内容。本地 JVM/TCP/归档/界面验证不代表真机或 OEM 验收。

- [APK 安装](apk-installation.md)提供配对专用的准备、查询、取消与私有安装会话上传。
  全局最多一个活动请求，最多保存八条按认证 Mac 隔离的记录；大小与 SHA-256 校验后
  仍需手机批准和系统确认。持久提交意图、可验证清理及显式未知结果处理避免误报成功
  或自动重装；已处理旧记录的迟到非最终回调不能重新阻塞新请求。Mac 使用原生文件
  面板、独立认证上传连接和会话/视图代次。本地协议、生命周期与合成界面检查不代表
  真机安装或 OEM 兼容性通过；分发渠道评估仍待做，APK 导出使用独立能力。
...
- [应用列表](application-library.md)增加独立 Mac 侧栏与 Android 进程内共享开关。
  ...
  诊断库存。JVM/Swift 与合成原生界面覆盖本次增量；真机行为待验证；APK 导出使用独立授权与传输边界。
  安装由上述独立能力提供。

- [基础音乐管理](basic-music.md)作为第四个媒体分类，提供音频分页、[歌曲信息与搜索](music-metadata.md)、文件名排序、
  时长、原生面板/Finder 导入和队列批量导出。Android 提供显式音乐授权入口：
  API 33+ 独立音频读取，API 26–32 共享旧版读取，API 29+ 可新建应用拥有的音频。
  旧设备端无音乐根目录时显示不可用；沿用 MediaStore 不续传与取消清理规则。
  本地 provider/权限/队列/生命周期和合成原生界面有验证，音乐真机行为尚未验证；
  音乐预览需点击播放才开始有界读取，支持暂停、定位和结束后重播，关闭时释放读取
  与监听。配对协议与合成 WAV/M4A 覆盖本地行为；未支持格式保留下载入口。
  [音乐封面](music-artwork.md)已通过有界系统图片派生物接入列表和播放预览，逐次读取与发布前复核音频权限，旧版或无封面时保留音符。有界歌曲标题/歌手/专辑展示与搜索已本地实现，下载仍使用原文件名；真机播放/格式/封面兼容及专辑/歌手分组视图仍待完善，不新增 M1 设备通过声明。

- MediaStore 支持的视频预览现提供原生播放、暂停和拖动进度。MP4/QuickTime/M4V/3GPP
  容器复用配对认证下载协议，单源串行读取且每段最多 1 MiB，保持最初源指纹/大小、
  CRC/offset、逐 chunk 权限与原有窗口限制。预览关闭、替换、刷新、导航或授权失效
  后不能发布迟到 open/字节。AVFoundation 只接收随机资源 URL，外部媒体引用、alias
  与外部播放均禁用，DroidMatch 不写视频缓存。本地配对 TCP、生命周期与合成视频
  解码/播放/暂停/跳转检查覆盖此增强；真机播放、厂商/provider 表现及原有 M1
  阻塞项仍未通过验收。

- 文件行的主点击区域与独立操作按钮分离，主传输不可用时不再连带禁用仍有能力执行的重命名/删除按钮和右键菜单。原有写能力/类型校验、父视图忙碌限制、mutation 上下文复核及删除确认保持不变；选择模式提供准确的本地化无障碍提示。媒体网格原本已独立处理右键操作，因此未改动。这是本地界面证据，不新增真机运行记录。

- 文件/媒体刷新后即使没有可选条目，仍可用“完成”退出选择模式；进入选择仍要求存在可选条目，忙碌期间的禁用规则不变。仍可见但已失去原有可选能力的路径会从已选集合移除，选择模式不会自动关闭。这是本地界面恢复改进，不是新的权限授权或真机证据。

- Mac “正在连接”卡片新增“取消”入口，复用已有会话断开操作：取消并使旧连接失效，等待清理期间显示断开进度，随后无失败提示地回到 idle；迟到的已取消结果不能恢复 ready 界面。配对审批沿用既有控件。该界面入口不新增真机证据。
...
- Mac 发现现会给设备卡补充仅用于展示的真实商品名，同时把原始 model/product 保留为次行技术信息；同一连接状态组内按相同的屏幕显示名回退链排序，不再按隐藏的技术型号排列。具体别名不是针对机型的 Swift 逻辑，而是放在随 App 签名的版本化 JSON 数据表中，并以 128 KiB/128 条为上限。通用加载器要求精确 schema，任一归一化身份、展示文本、语言标签或无凭据 HTTPS 来源无效都会整表 fail closed。成品 App 只读取该签名主 bundle 资源，缺失或损坏就 fail closed；只有 SwiftPM 测试和命令行产品可使用模块 bundle，生成的编译机绝对路径回退不会进入产品路径。独立、有界、来源可审计的本地别名表会按 Mac 首选语言执行完整标签→地区→文字→基础语言回退，要求精确设备身份，拒绝重复匹配，并且只持久化 canonical name。SHARP 704SH 可离线解析为夏普唯一已审核的日文「シンプルスマホ4」；中英文不会得到编造的翻译，而是安全保留该原名。其他缓存未命中项通过无 Cookie、拒绝重定向的临时会话流式请求唯一固定的 Google Play 完整公开目录。独立 catalog-loader actor 执行 8 MiB、UTF-16LE、表头、行数和字段长度上限并构建有界索引，resolver actor 最多保留 64 个待查参数。匹配/缓存身份使用完整的 512-scalar 有界参数而不是 120-scalar UI 投影，原始商品名唯一后才进入安全投影。Core 不发送 serial 或逐设备搜索词，本地最多用参数元组 SHA-256 键保存 512 个安全 canonical name。Google 来源名称现以来源和有界核验时间进入 v3 缓存：未过期条目完全离线；只有来源已知且曾核验的过期 v3 旧值会立即显示，并在后台用同一完整目录核验。来源不明的 v2 条目迁移后保持未核验且不显示，直到当前审核别名或完整目录确认；畸形 v3 条目会在 resolver 初始化时即从本地清除。内存目录保留原始核验时间，后续新查询不会用旧下载伪造新鲜度。有效刷新会更新改名并移除已缺失或不再唯一的匹配；失败则在既有节流下只保留曾核验的安全旧值。当前审核别名始终优先，已移除的别名不会作为未核验缓存继续存在。解析后的名称通过同一项符合凭据字节上限的投影进入匿名连接 lease，并由认证会话标题、新配对记录、可信设备行和诊断概览共同使用。诊断页会把去重后的厂商/原始型号保留为次级技术信息；这项仅展示优先级不能改变协议或存储身份。旧记录成功重连后仅建立进程内 pairing-ID→名称显示覆盖，并刷新无机密可信列表，不新增钥匙串读写。pairing ID/缓存映射不进入 Presentation 或持久化，只有安全名称会投影到可信设备行。cache actor hop 后的 generation 复核会阻止并发断开复活旧 ready 状态，成功撤销的 tombstone 也会拒绝任何随后乱序到达的 remember。新增五项严格资源加载回归后，当时 Swift 库存为 483；另有三项诊断身份直接测试覆盖商品名优先、回退/去重和重新投影。本项只有本地自动化证据，不新增 current-tip 704SH 真机通过声明。
...
- 不依赖 protobuf 的产品目录 domain 类型、分页/搜索/排序 `AsyncRpcControlClient` listing 与内嵌错误/row/token 校验；可选 provider MIME 会规范为最长 127 字节的受限小写 ASCII 值，畸形 metadata 只降级为 `nil`，不会改变 row identity、capability 或授权。`DirectoryBrowserPresentationTypes` 负责不改变 remote identity 的 UI-only 文件名净化及独立浏览/上传投影，`DirectoryBrowserPolicy` 纯处理 direct-child/mutation/media/error 决策，MainActor `DirectoryBrowserModel` 则唯一持有 client/Task/generation、原子 refresh、可重试 load-more、旧 generation 拒绝、跨页去重和脱敏 Published 状态。创建、重命名、单删和批删在展示时捕获模型签发的 opaque context；导航、查询 load/refresh/load-more、授权失效、reset 和每次应用列表页都会轮换它。重命名/删除还要求完整捕获条目相等，批删要求整批精确相等，因此 stale 或同 path A→B→A 的旧确认会在 runner/RPC 前以固定脱敏说明拒绝，已准入任务仍能跨导航排空。纯 `DirectoryBrowserThumbnailState` 只持有缩略图 generation/FIFO/active-key/失败/缓存 transition；旧 generation 已准入请求在排空前仍占四项上限，但不能发布，缓存统一限制为 64 项和 8 MiB，且该纯值不持有 client、Task、权限判断或 Published 值。导航只取消旧 listing 并清空旧 generation 尚未准入的行缩略图，不取消已准入 mutation；同 path 完成刷新当前搜索/排序 query，其他 path 则丢弃旧结果/错误。全局切换分类或最后一个可见浏览 surface 离开时才清理排队派生任务、预览和缓存，并保留 listing/query/导航；旧 surface 离开不能失效另一可见窗口仍使用的派生 generation 或 preview。512 px 预览在缩略图队列外，可作为第五个 control request。每个可见浏览 surface 和预览 sheet 都持有模型签发且不可由条目/path 推导的进程内 opaque context，只有当前 context 能读取或清除预览状态；旧窗口 dismiss、导航、刷新或授权失效会收敛到固定“预览不可用”提示，不会串用另一窗口的图片或留下空转 spinner。同一模型仍最多一个实际预览请求在途，已准入请求继续排空。listing 分页与预览/缩略图有独立有效性，load-more 不会使当前预览永久停在 loading。不可读 root 不再发起 list，但独立可写的 root 仍保留直接上传。文件页另有 250ms 搜索 debounce、provider-side 名称/修改时间/大小升降序、已加载项目全选/清除与 stale selection 对账、稳定 path 多选/顺序批量删除、多文件下载防覆盖，以及共用同一可测准入策略的原生面板/Finder 多文件上传：按顺序接受 1–100 个非符号链接普通文件，拒绝规范化后重名和目标/媒体类型不匹配；部分入队失败会保留并明确提示已接受任务。列表/网格显示大小和本地化修改日期并提供能力受限的原生右键操作，MediaStore 图片/视频单响应限制 512 KiB
- 视频/音频列表只为 MIME 匹配 `video/*` / `audio/*` 且数值为正的条目携带
  additive MediaStore 毫秒时长；Android 对图片、相册、SAF、App Sandbox、
  未知和误分类条目保持零，Mac 再次 fail closed 为 `nil`，并在列表、网格与
  预览中显示为 `m:ss` 或 `h:mm:ss`。时长元数据本身不授予播放能力，
  也不新增真机证据。
...
- 产品启动器入口（`DroidMatchActivity`）：提供经过单测的下一步就绪摘要，控制 paired-required endpoint、处理配对审批、列出/撤销不含密钥的已配对 Mac 元数据、处理通知权限、仅在用户点击后授权/重选照片和视频，并同时显示实时“全部/受限/关闭”总览与照片/视频各自的“全部项目/已选项目/关闭”，以及管理 SAF 授权。总览和两条细分状态来自同一对实时权限读取，因此只拒绝一种媒体时不会仅显示含糊的“受限”。对端提供的 Mac 名称在进入配对批准、可信列表或撤销确认前会共用一个 UI-only 安全投影：NFC 归一化、折叠空白，移除控制符、Unicode format 与孤立 surrogate；若过滤后没有可见内容则固定显示 `Mac`。经认证的原始名称仍保留在 transcript 与加密凭据 metadata 中，撤销身份仍是 pairing ID。SAF 添加/释放现在以原始 `getPersistedUriPermissions()` 的精确 URI/mode 快照为事实源：非 tree 或 write-only 的 picker 结果不会调用持久化，产品 root 确认失败只释放本次新增 mode 并保留此前 mode，移除则重新读取并释放该 URI 的全部当前 mode，而不信任 UI 缓存的写能力。添加只有请求 mode 与产品 root 都确认后成功，移除只有后续快照确认该 URI 已无任何 mode 后成功；快照异常、畸形/重复状态、残留 mode 或无法证明的回滚只产生固定脱敏提示。列表不可读时，列表与顶部文件夹计数都会标记为暂不可用，并提供显式重试。撤销信任会关闭活动 USB 会话，diagnostics harness 命名仅保留在 debug source。配对重连 proof 成功后会单调更新凭据密文中的最近使用时间；写盘失败只记有界诊断，不会推翻正确认证。媒体 root 的 `can_read` 现按图片/视频实时权限生成并与 `can_write` 独立；该产品权限流程与 SAF 授权事务强化只有本地 JVM/接线/构建证据，尚无真机 UI 归档。
...
- Android 配对 vault 目录现在会逐条完成结构与 AES-GCM tag 验证后才发布元数据。格式推导出的 484 字符编码上限会在整块 Base64 分配前检查；超大精确键值仍按可清理的结构损坏处理。可确定的单条畸形记录不再遮蔽显式 catalog 同轮健康记录；旧 list API 遇到不完整目录仍整体 fail closed。launcher 会标记目录不完整、隐藏全部未认证 payload 字段，并且只对严格小写 pairing-ID 后端键及其精确观测密文值、持久逐记录 revision 和损坏分类提供需二次确认的异常清理。save、revoke 与条件清理会在同一次 SharedPreferences commit 中推进 revision/tombstone；缺失 revision 的旧记录按零读取并在首次 mutation 时迁移。清理先封闭 endpoint 准入和 socket，并在 1.5 秒内确认全部已准入 RPC worker 退出；超时或失败会保留记录，并阻止替换 service/endpoint 启动，直到后续重试完成排空。随后重新扫描损坏分类，以值与 revision 的原子 compare-and-remove 删除未变化项，并在报告成功前复读完整目录。旧令牌不能删除健康、变化或精确值 A→B→A 重建记录；GCM 令牌的健康 key 见证消失后也会失效。非严格键、无效后端 value 类型及畸形/耗尽 revision 只会使目录不完整，不获得清理能力。若同轮没有另一条成功认证记录，GCM 失败仍按 vault/Keystore 状态未知整体 fail closed，不提供清理；认证 lookup 继续 fail closed。本项只有聚焦 JVM/资源/assemble 本地证据，不新增真机 Keystore 或 UI 声明。
...
- schema-v1 诊断导出器现在会在公开快照输入进入导出边界时再次校验：外部文本保持有界并
  ...
  发布签名声明。
- 原 768 行传输 scheduler actor 现以 751 行继续持有存活 task/record/queue、持久化副作用、timer 与发布。120 行纯 execution-event policy 校验 retry attempt、明确 retry 写盘失败回滚、只接受总量稳定的单调进度，并只让当前运行 rate generation 过期；它不持有 task、timer、store、queue、continuation、socket 或 broadcast。四项直接测试使 Swift 库存增至 431；既有 68 行 completion policy 继续对账 executor 退场。本项不新增真机证据。
...
- 4 MiB envelope、256 KiB 默认/1 MiB 最大 chunk、4 chunk/2 MiB 在途限额现由双端各自唯一的命名常量维护，并由 spec gate 与协议文档做一致性校验。Mac frame codec/reader 只接受 `1...4 MiB` 配置，以类型化错误取代整数转换 trap，同时不允许扩大协议准入边界；Android 会把高位长度前缀按无符号数解析后再拒绝。默认值和线格式均未改变，也不新增真机证据。
- 当前源码库存为 568 项 Swift 测试与 346 项 Android JVM 测试。Android 配对倒计时仍在独立且从无障碍树隐藏的控件中正常显示；阶段专用 polite live region 只在关闭、等待、待批准、已批准、已拒绝等真实变化时更新，不使用 Android 16 已弃用的主动 announcement API。等待批准时 SAS 作为六个独立 ASCII 数字朗读，500 ms 轮询中的未变化阶段/客户端/配对码写入会被抑制。这些计数与下述脚本事务回归只属于本地证据，不新增真机无障碍、Developer ID 或公证结果。
...
- 吞吐失败诊断路径仍只有离线工具覆盖；尚未归档真实的 `m1-adb-throughput-diagnostic-v1` fixture。后续 clean Slot A 媒体权限记录属于普通真机证据，不会关闭任何剩余 M1 阻塞项
...
- Slot A（SHARP 704SH，API 26）：已归档满足槽位要求的 handshake/list 和 clean current-tip 媒体权限撤销证据；两次功能完成的 100MiB 恢复探针使用旧 debug/Onone Mac harness，且早于当前传输优化，因此低于 20 MiB/s 的结果只是历史诊断，不是 current-tip gate 证据
...
截至 2026-09-22，`fixtures/m1-runs/` 包含：
- 90 个测试结果日志
...
- 通过：SHARP 704SH Slot A 以 clean rebuilt current-tip `m1-device-smoke-v1` 在 API 26 撤销 `READ_EXTERNAL_STORAGE` 后，`dm://media-images/` 稳定返回 `permissionRequired`，随后恢复原授权；结果归档自精确源码 `39d7f85`
