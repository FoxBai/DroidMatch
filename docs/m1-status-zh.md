...
- 并发：稳定 M1 probe 与产品异步 core 都已有受限的双流路径
  ...
  - 双流/混合流 probe 均可由脚本调用；下载与 provider-aware 上传 scheduler 已装配进认证后的视觉 target，具备按设备隔离持久化和按生命周期暂停；sandbox bookmark 重新获取与归档产品真机证据仍未完成
...
- 启用 App Sandbox 前补齐 security-scoped bookmark 捕获与重新获取；当前 ad-hoc 非 sandbox 产品已拥有私有 manifest 位置和断开生命周期
- 设置/偏好
- 通知集成
...
- **已有认证持久双向传输产品路径，但还不是完整管理器：** 本地化 SwiftUI target 已通过 serial 脱敏的异步边界发现设备，负责动态 forward 回收，执行 SAS 配对或 Keychain-backed 双向 proof，并仅在认证后开放真实分页文件浏览、隐私受限结构化诊断、原生文件面板和按设备身份隔离的下载/上传队列。未来 sandbox 分发所需的 security-scoped bookmark 重新获取仍未完成；产品路径已有本地测试和界面检查，但尚无归档真机配对/重连/传输证据。没有已配置的完整 Xcode 环境，因此 Developer ID 签名、公证和 DMG 尚未验证。
