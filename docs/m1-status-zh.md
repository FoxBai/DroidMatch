...
- 不依赖 protobuf 的产品目录 domain 类型、分页/搜索/排序 `AsyncRpcControlClient` listing、内嵌错误/row/token 校验，以及支持原子 refresh、250ms 搜索 debounce、provider-side 名称/修改时间/大小升降序、稳定 path 多选/顺序批量删除、多文件下载防覆盖、Finder 多文件拖放上传、部分失败对账、可重试 load-more、旧 generation 拒绝、跨页去重和脱敏失败状态的 MainActor `DirectoryBrowserModel`；MediaStore 图片/视频支持列表/自适应网格、可见项缩略图和 512 px 点击预览，单响应限制 512 KiB、列表缓存限制 64 项
