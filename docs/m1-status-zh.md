...
- SwiftUI `DroidMatch` 产品 target：中英文设备总览、按 canonical path 本地化内置 provider 根、异步 ADB 发现、进程内 opaque 设备 ID、旧快照提示、生成式原生图标，以及已验证的本地 ad-hoc `.app` bundle
...
- 不依赖 protobuf 的产品目录 domain 类型、分页/搜索/排序 `AsyncRpcControlClient` listing、内嵌错误/row/token 校验，以及支持原子 refresh、250ms 搜索 debounce、provider-side 名称/修改时间/大小升降序、稳定 path 多选/顺序批量删除、多文件下载防覆盖、Finder 多文件拖放上传、部分失败对账、可重试 load-more、旧 generation 拒绝、跨页去重和脱敏失败状态的 MainActor `DirectoryBrowserModel`；列表/网格显示大小和本地化修改日期并提供能力受限的原生右键操作，MediaStore 图片/视频另有可见项缩略图和 512 px 点击预览，单响应限制 512 KiB、列表缓存限制 64 项
...
- **已有认证持久双向传输产品路径，但还不是完整管理器：** 本地化 SwiftUI target 已具备 serial 脱敏发现、动态 forward、SAS/Keychain 认证、文件浏览、诊断、原生面板、设备隔离队列和 App 自有 bookmark 租约。带 sandbox entitlement 的 bundle 已通过内置 adb 发现 704SH 与 MEIZU M20，未观察到 sandbox deny；该 bundle 下的配对/重连/文件传输证据仍未归档。压缩本地 DMG、Applications 快捷方式、SHA-256、只读挂载及挂载后 App 复核已实现；Developer ID 签名与公证尚未验证。
