...
- 原 783 行产品文件浏览器父视图现以 767 行继续持有 SwiftUI 状态、原生面板、mutation 与队列提交；未改行为的列表/网格渲染归入 140 行无状态 state/actions 组件。93 行 Presentation 纯值独占选择模式/path 对账、按 capability 全选、按行序投影以及仅扣除已受理批量项，不持有 model、Task、panel 或 queue；三项直接测试覆盖该状态。118 行 AppSupport 纯搜索状态在统一 busy 生命周期中只保留最后一次 debounce 输入并拒绝目录或排序漂移；三项直接测试覆盖延迟准入、旧 token 与取消。137 行 AppSupport 纯策略另在面板完成时复核精确 query/row/授权/readiness，并让单项/批量下载共用本地 file URL、已存在目标及 canonical/case/width 重名预检。五项直接测试覆盖该边界；这只是本地证据。
- 原 774 行目录浏览 MainActor 现以 732 行继续持有 Published/listing/导航状态、派生 Task、预览、权限判断、mutation context 轮换及按 path 应用 mutation 结果。57 行预览状态边界持有进程内 opaque preview 与可见 surface context 及固定 loading/ready/unavailable/invalidated 展示状态。132 行纯缩略图状态独占 generation/FIFO/active-key/失败/缓存 transition，并让排空中的旧请求继续计入四项上限；三项直接测试覆盖旧 generation 并发、去重/可见性/失败准入及缓存双上限。157 行 MainActor runner 另独占活跃远端 mutation Task 与操作身份且不持有展示或刷新策略。三项 focused preview-context 回归覆盖跨窗口发布/旧 dismiss、旧 surface 离场、同 path refresh、导航、授权失效、单请求准入和 drain-safe 完成；三项 focused mutation-context 回归覆盖目录替换、完整条目替换与精确 A→B→A 回摆且零 RPC；已准入后导航回归继续通过。本项只有本地证据，不新增真机声明。
...
- 当前源码库存为 510 项 Swift 测试与 283 项 Android JVM 测试。三项 preview-context 回归证明共享浏览模型的窗口不会跨 context 发布、清除预览，也不会让旧 surface 离场失效当前预览、同 path refresh 不能复用旧 context、导航与授权失效会进入固定不可用状态，且已准入请求仍按单飞顺序排空。三项纯搜索准入回归证明文件/媒体浏览器在 listing、mutation 或 transfer submission 忙碌时只保留最后一次 debounce 输入，界面恢复可操作后立即提交，并在导航或排序改变后拒绝旧输入；这些测试都不新增 provider 或真机证据。Android 配对倒计时仍在独立且从无障碍树隐藏的控件中正常显示；阶段专用 polite live region 只在关闭、等待、待批准、已批准、已拒绝等真实变化时更新，不使用 Android 16 已弃用的主动 announcement API。等待批准时 SAS 作为六个独立 ASCII 数字朗读，500 ms 轮询中的未变化阶段/客户端/配对码写入会被抑制。这些计数与下述脚本事务回归只属于本地证据，不新增真机无障碍、Developer ID 或公证结果。
