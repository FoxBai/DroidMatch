...
| Android upload commit/cleanup state machines live outside the provider facade | `DmFileProvider` should remain the logical-path, permission, and catalog router. Package-private provider writers own ordered chunk boundaries plus app-sandbox atomic replacement, SAF temporary-document rename/cleanup, and MediaStore pending-row publication/cleanup; they do not parse RPC or authorize destinations. This is the first downward-only split of the provider monolith. |
