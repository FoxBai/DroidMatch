...
| Product transfer scheduling is a process-local FIFO actor capped at two running jobs | Coordinator correctness should remain independent of UI lifetimes. A buffering-newest snapshot stream gives native views observable state without unbounded history; progress uses monotonic receiver-confirmed offsets so reconnects never display sent-but-unacknowledged bytes or move backwards. Queued/running cancellation and terminal outcomes stay deterministic. Cross-restart queue persistence remains a separate post-M1 decision. |
