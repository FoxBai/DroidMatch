...
| Upload providers receive transfer ids before SAF resume opens | `OpenTransferRequest.transfer_id` reaches the Android upload provider layer so future SAF partial documents can use a durable protocol key instead of deriving state from user-visible display names. |
