...
<!-- source-size-max production=mac/Sources/DroidMatchApp/ProductFileBrowserView.swift:761 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
...
and path-gated mutation outcome application in 708 lines. A 46-line preview-state
boundary owns process-local opaque contexts and fixed loading/ready/unavailable/
...
preview-context regressions cover cross-window publication and stale dismissal,
same-path refresh, navigation, authorization invalidation, one-request admission,
and drain-safe completion.

中文：原 774 行 `DirectoryBrowserModel.swift` 现以 708 行继续持有 Published 状态、
listing generation、导航、派生 Task/预览/权限判断及按 path 应用 mutation 结果。46 行
预览状态边界持有进程内 opaque context 及固定 loading/ready/unavailable/invalidated
展示状态。132 行
...
发布/旧 dismiss、同 path refresh、导航、授权失效、单请求准入与 drain-safe 完成。
...
production source is `ProductFileBrowserView.swift` at 761 lines, the largest
...
中文：以上机器校验 marker 是当前工作树的权威值：最大生产源码为 761 行的
