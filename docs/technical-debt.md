...
|---|---|---|
| Large source files | **Partially reduced** | `AsyncTransferScheduler.swift` was reduced from 1,074 to 914 lines. `DmFileProvider.java` was reduced from 3,105 to 2,280 lines by extracting provider transfer-I/O state and the app-sandbox catalog. Four production files still require explicit debt ceilings. |
...
| File | Ceiling |
|---|---:|
| `android/app/src/main/java/app/droidmatch/m1/DmFileProvider.java` | 2,280 |
...
1. **Android provider I/O and catalogs (in progress):** upload writers, download
   reader mechanics, opaque-ID hashing, and the app-sandbox catalog are now
   separate; next extract MediaStore and SAF catalogs behind the existing
   `DmFileProvider` contracts. Preserve path, permission, atomic-commit, and
   resume behavior with the full Android gate.
