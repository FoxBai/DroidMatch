...
<!-- source-size-max production=mac/Sources/DroidMatchCore/AsyncTransferScheduler.swift:718 test=android/app/src/test/java/app/droidmatch/m1/AdbEndpointTest.java:601 -->
...
4. **Mac async router (default-budget reached):** `AsyncRpcRoutingState` owns
   ...
   718-line scheduler actor retains live record/queue, persistence-write,
   broadcast, and executor-unwind ownership. Its 49-line actor-confined
   rate-expiry state owns only timer replacement/cancellation and cannot mutate
   a job or publish a snapshot.
