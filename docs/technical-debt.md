...
<!-- test-inventory swift=492 android-unit=244 -->
...
492 Swift tests and 244 Android JVM tests; these offline regressions add no
physical-device or release-signing evidence.
The takeover baseline therefore names 492 Swift tests and 244 Android unit tests/lint;
...
异常节点会保留。当前库存为 492 项 Swift 测试与 244 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 492/244. Counts and
...
`DmFileProviderSafTest.java`，最大工具为 800 行的 `test-run-m1-throughput-gate.sh`，
测试库存为 492/244。下方拆分历史中嵌入的
