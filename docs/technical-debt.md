...
<!-- test-inventory swift=288 android-unit=177 -->
...
transfer retry-client gate 现有三项确定性测试：真实 TCP/配对认证覆盖生产 endpoint 与 credential 路径，失效前拒绝且不调用 connector，actor-held connector 则证明建连完成与失效竞态时会先关闭该 socket 再返回取消。测试接缝仅在模块内部可见，生产仍使用 lease endpoint 和固定 10 秒超时。当前 Swift 测试总数为 288。
