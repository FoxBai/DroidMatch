...
<!-- test-inventory swift=504 android-unit=286 -->
...
path-gated mutation outcome application, and an opaque cross-window search token
in 662 lines. The token contains no query, path, or search text; a newer edit
invalidates an older window's deadline, while token-scoped cancellation cannot
cancel the newer edit. A 132-line pure
...
中文：原 774 行 `DirectoryBrowserModel.swift` 现以 662 行继续持有 Published 状态、
listing generation、导航、派生 Task/预览/权限判断、按 path 应用 mutation 结果及不含
...
504 Swift tests and 286 Android JVM tests; these offline regressions add no
...
The takeover baseline therefore names 504 Swift tests and 286 Android unit tests/lint;
...
504 项 Swift 测试与 286 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 504/286. Counts and
