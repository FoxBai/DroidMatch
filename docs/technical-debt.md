...
|---|---|---|
| Large source files | **Unified budget enforced** | Every handwritten production and test Swift/Java/Kotlin file is at most 850 lines, with no exception; the largest is now an 847-line mixed-transfer test server. The former 2,526-line Mac frame/RPC fixture, 1,288-line multiplexer test, 1,173-line Android provider test, and 1,977-line dispatcher test are split by behavior/fixture ownership. Scheduler tests separate behavior from shared construction, framed-server response values are isolated, and the product file-browser toolbar is a stateless action/state boundary. |
...
| Single-maintainer risk | **Mitigated, not eliminated** | `AGENTS.md`, the current-state contribution guide, required PR handoff template, bilingual live docs, deterministic gates, 207 Swift tests, 119 Android unit tests/lint, and the model-verified review wrapper reduce undocumented knowledge. CI now rejects removal or drift of the takeover sections, physical-device boundary, 850-line contract, and PR evidence checklist. Ownership and release authority are still concentrated in one GitHub owner. |
...
中文结论：生产与测试代码现已统一执行 850 行门禁，存量测试巨石也已按行为和 fixture 所有权拆分；双下载并发证据已迁移到产品 async router，单人维护风险仍只有部分治理；Mac 已接通认证会话、文件浏览、结构化诊断、持久双向队列和 bookmark 租约，带 entitlement 的实包也已通过两台真机只读发现，但仍缺产品认证/传输与 sandbox 文件传输证据；Android 已升级为安全连接 onboarding/status 与 SAF 授权管理入口，但完整本地文件浏览体验仍未完成。

## Source-size Guardrail

`python3 tools/check-source-size.py` applies one 850-line ceiling to handwritten
