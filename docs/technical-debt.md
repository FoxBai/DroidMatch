...
<!-- test-inventory swift=472 android-unit=242 -->
...
472 Swift tests and 242 Android JVM tests; these offline regressions add no
physical-device or release-signing evidence.
The takeover baseline therefore names 472 Swift tests and 242 Android unit tests/lint;
...
异常节点会保留。当前库存为 472 项 Swift 测试与 242 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`check-maintainer-contract.py` at 800 lines, and the inventory is 472/242. Counts and
