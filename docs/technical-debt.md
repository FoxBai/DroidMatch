...
| Single-maintainer risk | **Mitigated, not eliminated** | `AGENTS.md`, the current-state contribution guide, required PR handoff template, bilingual live docs, deterministic gates, 207 Swift tests, 119 Android unit tests/lint, and the model-verified review wrapper reduce undocumented knowledge. CI now rejects removal or drift of the takeover sections, physical-device boundary, 900-line contract, and PR evidence checklist. Ownership and release authority are still concentrated in one GitHub owner. |
...
`python3 tools/check-source-size.py` applies one 900-line ceiling to handwritten
