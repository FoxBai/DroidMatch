...
<!-- test-inventory swift=510 android-unit=286 -->
...
510 Swift tests and 286 Android JVM tests; these offline regressions add no
...
The takeover baseline therefore names 510 Swift tests and 286 Android unit tests/lint;
...
510 项 Swift 测试与 286 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
production source is `ProductFileBrowserView.swift` at 767 lines, the largest
test source is `ProviderSafDocumentCacheTest.java` at 779 lines, the largest tool is
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 510/286. Counts and
