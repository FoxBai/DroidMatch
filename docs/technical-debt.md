...
<!-- source-size-max production=android/app/src/main/java/app/droidmatch/m1/DmFileProvider.java:662 test=mac/Tests/DroidMatchCoreTests/LocalFrameTestServer.swift:586 -->
<!-- test-inventory swift=311 android-unit=180 -->
...
1. **Android provider facade (default-budget reached):** upload writers,
   ...
   canonical upload-destination exclusion across sessions. The 662-line facade retains catalog
   contracts, the shared lease registry, the typed cache lifetime, and the public
   composition/delegation surface. The 41-line `ProviderSafDocumentCache` alone owns
