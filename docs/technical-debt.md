...
<!-- test-inventory swift=507 android-unit=283 -->
...
and an opaque cross-window search token in 751 lines. The token contains no query,
...
507 Swift tests and 283 Android JVM tests; these offline regressions add no
physical-device or release-signing evidence.
The takeover baseline therefore names 507 Swift tests and 283 Android unit tests/lint;
...
507 项 Swift 测试与 283 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 507/283. Counts and
