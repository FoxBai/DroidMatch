...
<!-- source-size-max production=mac/Sources/DroidMatchApp/ProductFileBrowserView.swift:800 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
<!-- tool-size-max path=tools/test-run-m1-throughput-gate.sh:800 -->
<!-- test-inventory swift=568 android-unit=346 -->
...
Two new Swift and four new JVM tests plus extended existing cases cover the added
behavior; native UI uses synthetic entries. The following playback increment
adds four Swift tests and extends audio/video range and preview cases. Native
WAV/M4A decode/pause/seek/replay, malformed-content replacement and stale-context
closure have local coverage. Audio shares the bounded source/loader/controller;
its progress observations and pending seeks end with the preview.
[Music artwork](music-artwork.md) now has separate typed-derivative input, source
geometry, encoded output and native Mac decode budgets. Three JVM and two Swift
tests plus extended existing RPC/preview cases cover the changed boundaries.
Indexing and actual device/codec/artwork observations remain open. See [Basic Music](basic-music.md).
...
queue, persistence-effect, timer, and publication ownership in 751 lines. Its
post-persistence control effects live beside the pure control policy, while a
519-line active-upload cancellation controller owns only the exact live transfer
handle and its cancel/final-ACK race. A
...
Focused local regressions cover the fresh MediaStore product-cancel
boundary: exact-ID wire cancellation before settlement, typed failure with
same-route retry and no refill, final-ACK precedence with authoritative post-ACK
source validation, executor/coordinator
pre-open failures, cleanup-unverified transport loss, and shutdown/suspension
without a false cancellation or automatic replay. This closes the
previous Task-cancellation shortcut in the Mac product queue, but it adds no
physical-device evidence for provider cleanup.

中文：原 768 行 `AsyncTransferScheduler.swift` 现以 751 行继续唯一持有存活 Task、
record、queue、持久化副作用、timer 与发布；写盘后的控制副作用归入同一
control-policy 边界，519 行活动上传取消 controller 只持有精确的存活 transfer
handle 及 cancel/final-ACK 竞态。120 行纯
...
专项本地回归覆盖 fresh MediaStore 产品取消边界：结算前的
精确 ID wire cancel、类型化失败后保留同 route 重试且不 refill、final ACK
优先且 ACK 后源身份复核保持权威、executor/coordinator 的 open 前失败、session 丢失后 cleanup-unverified，
以及 shutdown/suspension 不伪报取消且不自动重放。
这关闭了 Mac 产品队列中原来的 Task-cancel 捷径，但不新增 provider 清理的
真机证据。
...
mutation-context rotation, path-gated mutation outcome application, and an opaque
cross-window search token in 795 lines. The token contains no query, path, or search text; a newer edit
invalidates an older window's deadline, while token-scoped cancellation cannot
cancel the newer edit. A 57-line preview-state boundary owns process-local opaque
preview and visible-surface contexts plus fixed loading/ready/unavailable/invalidated
presentation states. A 132-line pure
...
separately owns the active remote-mutation Task and operation identity. Three direct
preview-context regressions cover cross-window publication, stale dismissal and
stale-surface departure, same-path refresh, navigation, authorization invalidation, one-request admission,
and drain-safe completion.

中文：原 774 行 `DirectoryBrowserModel.swift` 现以 795 行继续持有 Published 状态、
listing generation、导航、派生 Task/预览/权限判断、mutation context 轮换、按 path 应用 mutation 结果及不含
query/path/搜索文字的跨窗口 token；较新输入会使旧窗口 deadline 失效，按 token 取消也
不会取消较新输入。57 行预览状态边界持有进程内 opaque preview/可见 surface context 及固定
loading/ready/unavailable/invalidated 展示状态。132 行
...
runner 另独占活跃远端 mutation Task 与操作身份。三项 preview-context 回归覆盖跨窗口
发布/旧 dismiss、旧 surface 离场、同 path refresh、导航、授权失效、单请求准入与 drain-safe 完成。
...
Current-format credential selection now isolates malformed unrelated metadata
when a fully validated exact fingerprint match exists, reading only that target
secret and binding its decoded pairing ID back to the selected Keychain account.
If no valid exact match exists, any malformed current item returns the fixed
`invalidStoredRecord` failure before legacy secret reads or first-pairing fallback.
Four deterministic regressions cover unrelated corruption with a valid target,
corruption with no target, a corrupt target beside valid unrelated metadata, and
a selector-account/secret-pairing-ID mismatch, bringing the current Swift
inventory to 505 without changing display listing or legacy migration semantics.
This is local automated evidence only.

中文：当前格式凭据选择在存在完整校验的精确指纹匹配时，会隔离其他记录的畸形元数据，且
只读取目标机密，并把解码后的 pairing ID 绑定回所选 Keychain account；若不存在有效精确
匹配，任何已见当前格式畸形项都会在旧记录机密读取或首次配对回退前返回固定
`invalidStoredRecord`。四项确定性回归覆盖“无关损坏但目标有效”、“损坏且无目标”、
“目标损坏但无关记录有效”和“selector account 与机密 pairing ID 不一致”，使当前 Swift
库存增至 505；展示列表和旧格式迁移语义不变。这只是本地自动化证据。

The former 783-line `ProductFileBrowserView.swift` now keeps SwiftUI state,
native panels, mutations, and queue submission in 726 lines. Its unchanged
...
434. A 118-line pure AppSupport search state retains only the latest debounced
edit across the shared busy lifetime and rejects context drift; three direct
tests cover delayed admission, stale tokens, and cancellation. The existing
session-lifetime model test also covers global latest-token admission, stale
window cancellation, and context invalidation without increasing inventory. A 135-line
AppSupport policy separately fail-closes native-panel completion
...
中文：原 783 行 `ProductFileBrowserView.swift` 现以 726 行继续持有 SwiftUI 状态、
...
118 行 AppSupport 纯搜索状态在统一 busy 生命周期中只保留最后一次 debounce 输入并拒绝
上下文漂移；三项直接测试覆盖延迟准入、旧 token 与取消。既有会话生命周期测试另覆盖
全局最新 token、旧窗口取消及上下文失效，测试库存不变。137 行 AppSupport 纯策略另在
面板完成时复核精确 query、row 快照、授权与持久化
...
Current notification delivery closes the opt-out race around the asynchronous
macOS authorization lookup. A terminal event must observe the preference enabled,
and immediately before `UNUserNotificationCenter.add` the App compares that
event-generation snapshot with the current MainActor-owned preference generation
and live authorization through one pure policy. Disabling notifications while the
lookup is outstanding therefore suppresses the late enqueue; disabling and then
re-enabling also cannot revive a candidate from the older generation. One direct
policy test covers every rejected boundary and the sole passing combination,
raising the current Swift inventory from 488 to 489; this is local logic evidence
and does not claim a real system-permission interaction.

中文：当前通知投递已关闭 macOS 异步授权查询附近的关闭竞态。终态事件必须先观察到
偏好已开启；紧邻 `UNUserNotificationCenter.add` 前，App 还会通过同一个纯策略比较事件
代次快照、MainActor 持有的当前偏好代次与实时授权。因此查询尚未返回时关闭通知，会
抑制迟到入队；关闭后再次开启也不会复活旧代次候选。一项直接策略测试覆盖所有拒绝边界
和唯一通过组合，使当前 Swift 库存从 488 增至 489；这只是本地逻辑证据，不声称完成了
真实系统权限交互。
...
mutations and pagination cannot strand a preview. Shared-window preview sheets now
consume only their model-issued process-local opaque context; stale dismissal,
navigation, refresh, or authorization invalidation settles to fixed unavailable
state, and one model still drains at most one admitted preview request at a time.
Nine focused regressions cover
...
分页不会使预览永久停在 loading。共享窗口的预览 sheet 只消费模型签发的进程内
opaque context；旧 dismiss、导航、刷新或授权失效会进入固定不可用状态，同一模型仍
最多排空一个已准入预览请求。九项回归测试覆盖准入前取消、只读迟到响应
...
Unexpected coordination or cleanup nodes are preserved. Product download
publication now carries explicit `.mustBeAbsent` authority from native-panel
planning through schema-v3 persistence/resume and both writer checks. Targets
...
533 Swift tests and 313 Android JVM tests; these offline regressions add no
...
The takeover baseline therefore names 568 Swift tests and 346 Android unit tests/lint;
the older counts in the decomposition history remain milestone data.
Current executable test inventory is 568/346; 当前可执行测试库存为 568/346。
...
异常节点会保留。产品下载现把显式 `.mustBeAbsent` 权限从原生面板规划传到
schema-v3 持久化/恢复和 writer 两次复核；预检后或传输中出现的目标会原样保留，
schema-v1/v2 旧请求按不覆盖迁移，替换能力不进入产品数据源。当前库存为
533 项 Swift 测试与 313 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
the outer App resource seal binds the exact resulting bytes. Every other bundle failure remains
...
production source is `ProductFileBrowserView.swift` at 800 lines, the largest
test source is `ProviderSafDocumentCacheTest.java` at 779 lines, the largest tool is
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 568/346. Counts and
...
1. **Android provider facade (default-budget reached):** upload writers,
   ...
   validation and provider selection; `ProviderPathCoordinator` owns process-wide
   upload/cleanup/mutation target and ancestor exclusion across sessions. The facade retains
   the process-wide claim/cache composition and public delegation
   ...
   interfaces. `ProviderSafDocumentCache` alone owns root-scoped opaque token
   generation/resolution, synchronized access-order LRU state, listed parent/name
   metadata, provider-returned rename-ID rebinding, and fail-closed ancestor-chain
   reconstruction for SAF path claims; production shares its authority epochs across
   replacement facades while tests can inject isolated instances. Direct tests cover access
   refresh, eviction, changed-ID rename/delete invalidation, parent binding, tuple separation,
   cross-root rejection, and stale in-flight listing rejection.
   Its legacy size exception has been removed.
...
4. **Mac async router (default-budget reached):** `AsyncRpcRoutingState` owns
   ...
   and returns an explicit pause/interruption/terminal resolution. The 751-line
