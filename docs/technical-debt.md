...
|---|---|---|
| Large source files | **Partially reduced** | `AsyncTransferScheduler.swift` was reduced from 1,074 to 914 lines by extracting its product contract and retry relay. Four production files still require explicit debt ceilings. |
...
| File | Ceiling |
|---|---:|
| `android/app/src/main/java/app/droidmatch/m1/DmFileProvider.java` | 3,105 |
...
1. **Android provider I/O and catalogs:** extract upload writers, stream-reader
   mechanics, and app-sandbox/MediaStore/SAF catalogs behind the existing
   `DmFileProvider` contracts. Preserve path, permission, atomic-commit, and resume
   behavior with the full Android gate.
