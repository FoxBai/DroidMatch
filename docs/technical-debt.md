...
<!-- test-inventory swift=507 android-unit=286 -->
...
mutation-context rotation, atomic mutation issues, path-gated outcome application,
and an opaque cross-window search token cross-window search token
in 751 lines. The token contains no query,
path, or search text; a newer edit
invalidates an older window's deadline, while token-scoped cancellation cannot
cancel the newer edit. Opaque mutation contexts and complete item snapshots reject
stale or exact A→B→A confirmations before the runner/RPC while admitted work still
drains. A 132-line pure
...
中文：原 774 行 `DirectoryBrowserModel.swift` 现以 751 行继续持有 Published 状态、
listing generation、导航、派生 Task/预览/权限判断、mutation context 轮换、原子
mutation issue、按 path 应用结果及不含 query/path/搜索文字的跨窗口 token；较新输入会
使旧窗口 deadline 失效，按 token 取消也不会取消较新输入。opaque mutation context 与
完整条目快照会在 runner/RPC 前拒绝 stale 或精确 A→B→A 确认，已准入工作仍继续排空。132 行
...
native panels, mutations, and queue submission in 761 lines. Its unchanged
...
中文：原 783 行 `ProductFileBrowserView.swift` 现以 761 行继续持有 SwiftUI 状态、
...
507 Swift tests and 286 Android JVM tests; these offline regressions add no
...
The takeover baseline therefore names 507 Swift tests and 286 Android unit tests/lint;
...
507 项 Swift 测试与 286 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 507/286. Counts and
