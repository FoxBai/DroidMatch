...
<!-- test-inventory swift=505 android-unit=287 -->
...
mutation-context rotation, path-gated outcome application, and an opaque
cross-window search token opaque cross-window search token cross-window search token
in 795 lines. The token contains no query, path, or search text; a newer edit
...
listing generation、导航、派生 Task/预览/权限判断、mutation context 轮换、按 path 应用结果及不含 query/path/搜索文字的跨窗口 token；较新输入会
使旧窗口 deadline 失效，按 token 取消也不会取消较新输入。opaque mutation 结果及不含
...
505 Swift tests and 287 Android JVM tests; these offline regressions add no
...
The takeover baseline therefore names 505 Swift tests and 287 Android unit tests/lint;
the older counts in the decomposition history remain milestone data.
Current executable test inventory is 505/283; 当前可执行测试库存为 505/283。
...
505 项 Swift 测试与 287 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
