...
<!-- source-size-max production=mac/Sources/DroidMatchApp/ProductFileBrowserView.swift:795 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
<!-- tool-size-max path=tools/test-run-m1-throughput-gate.sh:800 -->
<!-- test-inventory swift=542 android-unit=313 -->
...
The takeover baseline therefore names 542 Swift tests and 313 Android unit tests/lint;
the older counts in the decomposition history remain milestone data.
Current executable test inventory is 542/313; 当前可执行测试库存为 542/313。
...
production source is `ProductFileBrowserView.swift` at 795 lines, the largest
test source is `ProviderSafDocumentCacheTest.java` at 779 lines, the largest tool is
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 542/313. Counts and
