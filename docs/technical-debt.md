...
<!-- source-size-max production=mac/Sources/DroidMatchApp/ProductFileBrowserView.swift:767 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
...
and path-gated mutation outcome application in 732 lines. A 57-line preview-state
boundary owns process-local opaque preview and visible-surface contexts plus fixed loading/ready/unavailable/
invalidated presentation states. A 132-line pure
...
中文：原 774 行 `DirectoryBrowserModel.swift` 现以 732 行继续持有 Published 状态、
listing generation、导航、派生 Task/预览/权限判断及按 path 应用 mutation 结果。57 行
预览状态边界持有进程内 opaque preview/可见 surface context 及固定 loading/ready/unavailable/invalidated
展示状态。132 行
...
native panels, mutations, and queue submission in 707 lines. Its unchanged
...
tests cover delayed admission, stale tokens, and cancellation. A 135-line
...
中文：原 783 行 `ProductFileBrowserView.swift` 现以 707 行继续持有 SwiftUI 状态、
...
上下文漂移；三项直接测试覆盖延迟准入、旧 token 与取消。137 行 AppSupport 纯策略另在
