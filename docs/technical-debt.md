...
<!-- source-size-max production=mac/Sources/DroidMatchCore/AsyncTransferScheduler.swift:739 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
<!-- tool-size-max path=tools/test-run-m1-throughput-gate.sh:800 -->
<!-- test-inventory swift=509 android-unit=287 -->
...
queue, persistence-effect, timer, and publication ownership in 739 lines. Its
post-persistence control effects live beside the pure control policy, while a
485-line active-upload cancellation controller owns only the exact live transfer
...
Seven focused local regressions cover the fresh MediaStore product-cancel
boundary: exact-ID wire cancellation before settlement, typed failure with
same-route retry and no refill, final-ACK precedence, executor/coordinator
...
中文：原 768 行 `AsyncTransferScheduler.swift` 现以 739 行继续唯一持有存活 Task、
record、queue、持久化副作用、timer 与发布；写盘后的控制副作用归入同一
control-policy 边界，485 行活动上传取消 controller 只持有精确的存活 transfer
...
七项专项本地回归覆盖 fresh MediaStore 产品取消边界：结算前的
精确 ID wire cancel、类型化失败后保留同 route 重试且不 refill、final ACK
优先、executor/coordinator 的 open 前失败、session 丢失后 cleanup-unverified，
...
509 Swift tests and 287 Android JVM tests; these offline regressions add no
physical-device or release-signing evidence.
The takeover baseline therefore names 509 Swift tests and 287 Android unit tests/lint;
...
509 项 Swift 测试与 287 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
production source is `AsyncTransferScheduler.swift` at 739 lines, the largest
test source is `ProviderSafDocumentCacheTest.java` at 779 lines, the largest tool is
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 509/287. Counts and
...
4. **Mac async router (default-budget reached):** `AsyncRpcRoutingState` owns
   ...
   and returns an explicit pause/interruption/terminal resolution. The 739-line
