...
|---|---|---|
| Large source files | **Partially reduced** | `AsyncTransferScheduler.swift` was reduced from 1,074 to 914 lines. `DmFileProvider.java` was reduced from 3,105 to 1,905 lines by extracting provider transfer-I/O state plus app-sandbox and MediaStore catalogs. Four production files still require explicit debt ceilings. |
...
| File | Ceiling |
|---|---:|
| `android/app/src/main/java/app/droidmatch/m1/DmFileProvider.java` | 1,905 |
...
1. **Android provider I/O and catalogs (in progress):** upload writers, download
   reader mechanics, opaque-ID/MIME helpers, app-sandbox, and MediaStore catalogs
   are now separate; next extract SAF behind the existing `DmFileProvider`
   contracts. Preserve path, permission, atomic-commit, and resume behavior with
   the full Android gate.
