...
<!-- test-inventory swift=465 android-unit=242 -->
...
only on API 35+, preserving the existing API 26–34 padding. This closes the known
target-SDK and edge-to-edge drift locally, but it is not API 35/36 device evidence.
...
继续沿用原 padding。已在本地收口 target SDK 与 edge-to-edge 漂移，但这不是 API 35/36
真机证据。
...
465 Swift tests and 242 Android JVM tests; these offline regressions add no
physical-device or release-signing evidence.
The takeover baseline therefore names 465 Swift tests and 242 Android unit tests/lint;
...
异常节点会保留。当前库存为 465 项 Swift 测试与 242 项 Android JVM 测试；这些只属于
离线回归，不新增真机或发布签名证据。
...
`check-maintainer-contract.py` at 800 lines, and the inventory is 465/242. Counts and
