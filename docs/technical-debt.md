...
secret. If no valid exact match exists, any malformed current item returns the
fixed `invalidStoredRecord` failure before legacy secret reads or first-pairing
fallback. Three deterministic regressions cover unrelated corruption with a valid
target, corruption with no target, and a corrupt target beside valid unrelated
metadata, bringing the current Swift inventory to 504 without changing display
listing or legacy migration semantics. This is local automated evidence only.

中文：当前格式凭据选择在存在完整校验的精确指纹匹配时，会隔离其他记录的畸形元数据，且
只读取目标机密；若不存在有效精确匹配，任何已见当前格式畸形项都会在旧记录机密读取或首次
配对回退前返回固定 `invalidStoredRecord`。三项确定性回归覆盖“无关损坏但目标有效”、
“损坏且无目标”和“目标损坏但无关记录有效”，使当前 Swift 库存增至 504；展示列表和旧格式
迁移语义不变。这只是本地自动化证据。
...
Current executable test inventory is 504/283; 当前可执行测试库存为 504/283。
