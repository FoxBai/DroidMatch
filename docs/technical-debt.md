...
<!-- source-size-max production=mac/Sources/DroidMatchPresentation/DirectoryBrowserModel.swift:795 test=android/app/src/test/java/app/droidmatch/m1/ProviderSafDocumentCacheTest.java:779 -->
<!-- tool-size-max path=tools/test-run-m1-throughput-gate.sh:800 -->
<!-- test-inventory swift=533 android-unit=313 -->
...
The takeover baseline therefore names 533 Swift tests and 313 Android unit tests/lint;
the older counts in the decomposition history remain milestone data.
Current executable test inventory is 533/313; 当前可执行测试库存为 533/313。
...
production source is `DirectoryBrowserModel.swift` at 795 lines, the largest
test source is `ProviderSafDocumentCacheTest.java` at 779 lines, the largest tool is
`test-run-m1-throughput-gate.sh` at 800 lines, and the inventory is 533/313. Counts and
...
中文：以上机器校验 marker 是当前工作树的权威值：最大生产源码为 795 行的
`DirectoryBrowserModel.swift`，最大测试源码为 779 行的
`ProviderSafDocumentCacheTest.java`，最大工具为 800 行的 `test-run-m1-throughput-gate.sh`，
测试库存为 533/313。下方拆分历史中嵌入的
