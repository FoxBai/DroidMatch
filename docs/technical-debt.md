...
| Synchronous Mac networking | **Nearly removed** | Product paths and every download probe now use `AsyncFramedTcpSession`/`AsyncRpcControlClient`, including full/partial download, sidecar resume, and transport retry. Synchronous `FramedTcpSession` remains only in full/partial upload, upload resume, and upload retry evidence. |
...
5. **Legacy synchronous removal (in progress):** all non-transfer network probes now
   ...
   every download evidence path now also uses that router. Remaining full/partial upload, upload resume, and upload retry evidence still use `FramedTcpSession`. Their migration needs equivalent local
