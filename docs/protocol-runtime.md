...
- Every SAF upload first creates or reopens a hidden partial document whose display name is derived from `transfer_id`, parent document id, requested final display name, and expected size.
- A non-final fresh close deletes its verified provisional document. A resumable upload keeps the partial so a later `upload --resume` can continue at the sidecar offset.
...
## Video and Music Playback Range Consumer

The Mac video and music player also consumes `OpenTransfer` downloads for bounded ranges,
...
中文：视频与音乐预览复用有界下载与最初源指纹，不修改 wire 或创建持久传输任务；
