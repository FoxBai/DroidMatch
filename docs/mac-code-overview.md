...
**AsyncTransferScheduler / policy / persistence** (`AsyncTransferScheduler.swift`, `AsyncTransferSchedulerPersistence.swift`, `AsyncTransferSchedulerPolicy.swift`, `TransferQueuePersistence.swift`)
...
- Publishes combined bookmark-registry/manifest `disabled`/`healthy`/`writeFailed` health without exposing filesystem paths or raw I/O errors, reloads authoritative health after submissions or queue mutations whose bookmark work may happen outside a scheduler snapshot, and routes an explicit retry through both stores
