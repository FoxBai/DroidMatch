...
an idle playback context until explicit Play; optional cover requests use the existing bounded thumbnail RPC. The shared native
...
`MediaArtworkImage` validates real JPEG/PNG headers and 512-pixel bounds before
pixel allocation; `MusicArtworkView` owns the current visible decoded cover and
falls back to a note. The browser retains sole ownership of encoded cache and
preview generations; see [Music artwork](music-artwork.md).
...
- Shares validation with `FrameReader`: both accept only `1...4 MiB`, so public
  configuration cannot trap during integer conversion or widen the documented
  M1 admission boundary
- Used for all ADB M1 communication

**RpcWireLimits** (`RpcWireLimits.swift`)
- Owns the named 4 MiB envelope, 256 KiB default/1 MiB maximum chunk, and
  4-chunk/2 MiB in-flight constants used across Core
- Is checked against Android and the protocol documents by the spec gate
...
- Validates the injected frame limit before constructing `NWConnection`, so an
  invalid codec configuration has no transport side effect
...
- Persists prepared resumable-upload cancellation as schema-v3 `cleanupPending`.
...
**DirectoryListing / AudioMetadata / DirectoryBrowserPolicy / DirectoryBrowserModel / MediaLibraryModel** (`DirectoryListing.swift`, `DroidMatchPresentation/DirectoryBrowser*.swift`, `MediaLibraryModel.swift`)
...
- Carries independently bounded title/artist/album through `AudioMetadata` only for readable canonical Audio files. `FileEntryMetadataHeading` and `MusicMetadataSummary` render tags with filename fallback, while operation naming remains unchanged. The preview hides captured headings after context invalidation; see [Music metadata](music-metadata.md).
...
- Keeps stable phase/failure/item values, an opaque search token with no query/path/text, independent `canBrowse` / `canAcceptUpload` projections, UI-only bidi/control-safe display names, opaque mutation tickets/targets, and pure operation-specific mutation guidance in a 236-line declaration boundary; the raw name and canonical identity remain unchanged for explicit operations
- Maps create, rename, single-delete, and batch-delete failures to distinct fixed localized guidance. The model publishes an atomic operation/category issue, so all windows observing the process-shared browser use the initiating operation rather than their own local sheet state. A synchronous create/rename admission rejection stays inside its still-visible edit sheet; admitted asynchronous failures return to the browser alert. Neither surface receives a path, item name, or raw exception
- 中文：创建、重命名、单删与批删使用各自固定的脱敏失败说明；模型原子发布操作/类别，因此观察进程级共享浏览器的所有窗口都使用发起操作，不会套用自己的本地 sheet 状态。同步准入拒绝留在仍可见的编辑 sheet 内，已准入后的异步失败回到浏览器提示，两者都不接收路径、条目名或原始异常
- Captures one model-issued opaque context when each create/rename/single-delete/batch-delete surface opens. Navigation, query load/refresh/load-more, authorization invalidation, reset, and every applied page rotate the module-private process-local identity. Item mutations also require the complete captured item values, so stale and exact A→B→A confirmations stop before the runner or RPC; an already admitted request still drains independently of later navigation
- 中文：创建、重命名、单删与批删在展示时捕获模型签发的 opaque context；导航、查询/刷新/翻页、授权失效、reset 及每次应用列表页都会轮换模块私有的进程内身份。条目 mutation 还要求完整捕获值相等，因此 stale 或同 path A→B→A 的旧确认会在 runner/RPC 前拒绝，已准入请求仍不受后续导航影响并正常排空
...
- Keeps admitted thumbnail/preview awaits alive until their real response or deadline instead of cancelling only the caller while Core still drains the wire request. A late permission failure remains fail-closed within the shared Images/Albums, Videos or Music authorization domain, but cannot erase a browser that has since moved to an unrelated provider path
- 中文：已准入的缩略图/预览会等待真实响应或 deadline，不会只取消 caller 而让 Core 在后台继续排空却提前释放名额；迟到权限错误只在 Images/Albums 共享域、Videos 或 Music 域内 fail closed，不会清除已经切换到无关 provider 的浏览器
...
- Distinguishes cancellation of the connect/pair UI task from a dependency-owned `CancellationError`: only the former stays silent, while the latter leaves every busy phase through the stable `connectionUnavailable` presentation path
- 中文：连接/配对 UI task 的真实取消与依赖自身抛出的 `CancellationError` 分开处理；仅前者静默，后者会从各忙碌阶段退出并映射为稳定的 `connectionUnavailable` 展示状态
...
- Keeps `FileEntryRow` secondary buttons and its context menu outside the primary button's disabled environment. The primary action remains inert when transfer admission is closed, while existing writable-item actions retain the parent's busy and mutation-context boundaries. Selection stays inside the primary button and announces Select; the media grid already has an independent context-menu boundary.
...
- Creates a standard `.app`, renders ten exact RGBA icon sizes, strictly packages modern ICNS chunks with no overwrite, and asks the platform decoder to reopen the result. The renderer maps bitmap, PNG, and output-write failures to fixed bounded bilingual errors without forwarding local paths or Swift backtraces. Because caller-selected unsigned custom adb remains available outside evidence mode, the input's pre-existing signature is not an authenticity boundary. The builder ad-hoc signs the copied nested executable and outer App with the hardened-runtime option, leaving the SDK source untouched. Nested/outer signing, complete candidate/final verification, and final-path adb execution remain fail-closed; the outer resource seal binds the resulting bytes. Evidence verification additionally requires the frozen signed-ADB profile. Its identity helper remains static-only for candidate and final checks; the final verifier executes the stable published bundle adb under a credential-free environment and checks the reviewed version/build, so a retry does not execute a newly copied private inode. The repository does not depend on the macOS 26.5 `iconutil` encoding path that rejects even a valid extracted iconset
...
- Retries `hdiutil verify` at most twice after the exact macOS `Resource temporarily unavailable` condition. Candidate validation covers the complete static tree, resources, signatures, entitlements, and evidence ADB static identity while deferring the private-transaction-path `adb version` launch; immediately after atomic publication, the final path receives the complete verifier and executes that stable embedded adb before the transaction can be marked complete, with replacement rollback or first-publication withdrawal on failure. Sandbox end-to-end mocks bind both verifier calls and all three adb signature outcomes; hard-kill recovery spans first install, verification, both sides of verified-state persistence, `rollback-required`, the rollback swap, and `rolled-back`. Published and mounted-App checks retry at most twice only for the exact `embedded adb is not runnable` result. Every published evidence-ready attempt revalidates the static ADB profile and reviewed runtime version; every mounted-App attempt retains the complete ordinary bundle, signature, entitlement, and runnable-adb boundary after that already-verified App is packaged. Malformed images and every other bundle or tool error fail immediately, and checksum publication remains after successful mount validation only
...
- Observes direct-child exit without reaping so the leader anchors its exact PID/PGID through bounded TERM/KILL escalation, then returns the ordinary timeout only after reaping and group cleanup are confirmed; the fixed cleanup-unconfirmed error exposes no path or PID, maps to product unavailability, and permanently latches that discovery instance against discovery or new-forward launches while still allowing one-shot removal of already-owned loopback forwards during session teardown
...
- Accepts an optional exact environment map: `nil` inherits the host environment, while an explicit empty map passes no variables.
- Used by `AdbClient`
