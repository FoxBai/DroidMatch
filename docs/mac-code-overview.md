...
- Keeps stable phase/failure/item values, independent `canBrowse` / `canAcceptUpload` projections, UI-only bidi/control-safe display names, opaque mutation tickets/targets, and pure operation-specific mutation guidance in a 224-line declaration boundary; the raw name and canonical identity remain unchanged for explicit operations
