...
- Keeps stable phase/failure/item values, an opaque search token with no query/path/text, independent `canBrowse` / `canAcceptUpload` projections, UI-only bidi/control-safe display names, opaque mutation tickets/targets, and pure operation-specific mutation guidance in a 234-line declaration boundary; the raw name and canonical identity remain unchanged for explicit operations
