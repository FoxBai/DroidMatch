...
- Keeps stable phase/failure/item values, an opaque search token with no query/path/text, independent `canBrowse` / `canAcceptUpload` projections, UI-only bidi/control-safe display names, and pure operation-specific mutation guidance in a 169-line declaration boundary; the raw name and canonical identity remain unchanged for explicit operations
