...
- Admits a running fresh-only MediaStore cancellation into non-terminal
  ...
  transfer ID while refill stays stopped. A final ACK observed first settles
  `completed`; session loss before cleanup confirmation becomes persistent
  `interrupted`/cleanup-unverified and is never automatically replayed
- 中文：运行中 fresh-only MediaStore 取消先进入非终态
  ...
  供重试。final ACK 先到以 `completed` 为准；清理确认前 session 丢失则
