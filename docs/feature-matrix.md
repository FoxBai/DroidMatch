...
| Music management | Optional | Required | Required | Basic list/search/sort/duration/import/export, live audio authorization, and native play/pause/seek are implemented with local evidence. [Bounded artwork](music-artwork.md) is implemented for provider-supplied API 29+ covers with a placeholder fallback. [Title/artist/album display and search](music-metadata.md) are implemented locally with filename fallback. Artist/album group views and device/codec/artwork validation remain open. |
| App list and APK install | Optional | Improved | Required | Scoped launcher metadata, search/sort and explicit sharing plus [single-APK system-confirmed installation](apk-installation.md) are implemented locally. [Single/split APK export](apk-export.md) is implemented locally; channel review and device validation remain open; see [Application Library](application-library.md). |
