...
- Product-facing transport APIs are async and cancellation-aware. Baseline `m1-smoke` also enters through `AsyncFramedTcpSession`; remaining M1 CLI probes may retain the synchronous adapter only for evidence compatibility. App/MainActor code must always enter through `AsyncFramedTcpSession` or a higher async abstraction.
