...
- `AdbDeviceDiscovery` is the first concrete product `DeviceDiscovery` boundary. It executes bounded blocking ADB listing on a private queue, retains serials only in a Core actor, emits process-local opaque UUIDs, and normalizes failures before `DeviceDiscoveryModel` reaches MainActor. The model atomically replaces successful snapshots, marks retained data stale after failure, and rejects late refresh generations.
