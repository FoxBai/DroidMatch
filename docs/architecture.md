...
- Product async downloads keep file ownership below the scheduler/UI boundary: a transfer handle serializes chunk → partial write → ACK, performs blocking file calls on a private serial queue, and publishes the completed partial only after final ACK. Product selection attaches an explicit `mustBeAbsent` publication policy that survives queue persistence and resume; writer acquisition and commit both reject an existing destination, and the final absent-target rename remains exclusive. A target created after panel preflight or during transfer is therefore preserved rather than replaced. Schema-v1/v2 manifests without the policy migrate fail closed to `mustBeAbsent`, while schema v3 records it explicitly and product restoration rejects replacement requests. Core compatibility/harness callers may still opt into `replaceExisting`. The scheduler owns sidecar/retry policy and must open with the inspected partial offset plus source fingerprint; the receiver validates the accepted offset again before writing.
...
- `DmFileProvider` is shared by concurrent Android sessions and owns one process-level `ProviderPathCoordinator`. Active uploads, exact resumable-partial cleanup, and create/rename/delete reserve the same logical target/ancestor claim set without waiting. Same-target and target/ancestor collisions fail as `alreadyExists`; provider I/O runs outside the coordinator monitor, while token-qualified release on open failure, write failure, final commit, cancel, mutation exit, or session teardown cannot free a replacement owner. App Sandbox claims use canonical logical segments. SAF claims bind authority, document ID, parent/name, and the ancestor chain proven by the opaque-token cache; an incomplete, ambiguous, or cyclic chain temporarily claims that SAF namespace as a fail-closed fallback. Multiple persisted roots sharing one authority also use that whole-authority claim because a narrower root cannot prove ancestors visible only through a broader grant. Per-session transfer IDs and two-stream limits remain separate RPC concerns.
...
- `DmFileProvider` owns logical root constants, provider composition/delegation, the bounded process-local SAF logical-token cache lifetime, and the shared path coordinator. `ProviderMediaCatalog`, `ProviderSafCatalog`, and `ProviderAppSandboxCatalog` define package-private storage ports plus fail-closed empty defaults; concrete Android catalogs implement those ports rather than facade-nested interfaces. `ProviderPagePolicy` owns pagination tokens, while `ProviderPathRouter` owns logical path/target validation and opaque SAF token resolution. `ProviderDirectoryListings` assembles point-in-time root read/write capability without treating it as an authorization token. `AppSandboxPathResolver` is the single stateless filesystem boundary for lexical validation, canonical-root confinement, and existing symbolic-link-component rejection across listing, mutation, and transfer opens; `AndroidAppSandboxCatalog` owns the remaining app-private provider behavior and staging lifecycle. `AndroidMediaCatalog` owns full/selected/denied media-access resolution, exact-item visibility checks for selected access, and MediaStore resolver calls, URI/query arguments, cursor lifetime, token cache, error mapping, thumbnails, pending rows, and transfer I/O; `AndroidSafCatalog` owns persisted tree/document listing, download, mutation admission, metadata validation, and their live permission/error mapping; `AndroidSafUploadOpener` owns authorized SAF destination/partial creation, exact child lookup, ACK-loss truncation, writer handoff, and pre-handoff cleanup. Its resolver-facing `SafUploadDocumentStore` requires non-null exact-name queries; an offset-zero resumable restart accepts deletion only when the provider returns true and a second exact query proves absence, then accepts creation only when a second query proves one ordinary file with the create-returned document ID. False/exceptional deletion, residual/duplicate names, auto-renaming, wrong kind, and unverifiable queries fail before writer handoff. Every upload stages under the reserved hidden `.droidmatch-upload-` prefix. Create/rename identities are canonicalized and reconciled with the unique exact-name child in the claimed parent before use; unverified returned identities are preserved rather than deleted or rebound, while later cleanup is limited to a previously verified provisional identity after rechecking parent/name/ID. SAF has no portable atomic no-replace rename, so an external provider actor can still race the final destination check until provider-specific device behavior is validated. `ProviderAuthorizedTransfers` revalidates catalog-supplied live authorization before each provider-backed chunk, resolves selected-media access against the exact active item, and closes denied handles, while `SafUploadWriter` repeats the exact tree check before final publication. `MediaStoreCursorReader` only scans already-open MediaStore cursors into typed pages/albums/lookups/metadata, and `SafDocumentCursorReader` does the same for six-column SAF item/metadata/child values. `SafDocumentPolicy` purely interprets document MIME/flags, stable ordering, and hidden partial names, while `SafUploadOpenPolicy` purely classifies fresh/restart/resume and validates decoded partial metadata; none of these stateless boundaries owns resolver, URI, live permission, descriptor, stream, cache, or cleanup state. Reader/writer helpers own transfer I/O state, while opaque-ID, MIME, and cleanup helpers centralize shared mechanics. None parses RPC envelopes.
- `MediaPermissionPolicy` owns pure API-level request sets, aggregate launcher state, per-image/video display classification, callback/fallback, and legacy MediaStore-write decisions. `DroidMatchActivity` derives the aggregate and both detail rows from the same pair of live reads; `MediaPermissionController` performs only user-triggered platform requests or guarded Settings navigation. `PermissionStateProvider` recomputes live grants for launcher, diagnostics, and provider capability reporting; none of these stores platform URIs or widens wire permission enums.
