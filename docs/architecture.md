...
- `DmFileProvider` owns logical roots, pagination tokens, the bounded process-local SAF logical-token map, and catalog dispatch. `ProviderPathRouter` owns logical path/target validation and opaque SAF token resolution. `AndroidAppSandboxCatalog` owns canonical app-private files; `AndroidMediaCatalog` owns live permission-aware MediaStore operations; `AndroidSafCatalog` owns persisted tree/document resolver calls, URI/cursor lifetime, permission/error mapping, mutations, transfer I/O, and transfer-ID partial resume. `SafDocumentCursorReader` only decodes an already-open six-column cursor into typed item/metadata/child values, while `SafDocumentPolicy` purely interprets document MIME/flags, stable ordering, and hidden partial names; neither owns resolver, URI, live permission, descriptor, stream, or cleanup state. Reader/writer helpers own transfer I/O state, while opaque-ID, MIME, and cleanup helpers centralize shared mechanics. None parses RPC envelopes.
