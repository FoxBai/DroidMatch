...
- Product-facing transport APIs are async and cancellation-aware. Baseline `m1-smoke` and ordinary `list-dir` also enter through `AsyncFramedTcpSession`; remaining M1 CLI probes may retain a synchronous adapter only for evidence compatibility. App/MainActor code must always enter through `AsyncFramedTcpSession` or a higher async abstraction.
