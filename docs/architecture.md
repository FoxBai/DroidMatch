...
- Product persistent restoration is two-phase: the scheduler reconstructs and canonicalizes its manifest behind an execution latch honored by every admission path, then exposes only the non-terminal local endpoint set to the platform access boundary. The coordinator activates queued work only after the bookmark provider reports both healthy durable state and complete target coverage. A corrupt, empty, or incomplete archive therefore keeps rows visible as `writeFailed` without starting an executor.
- `TransferQueueModel` explicitly starts/stops a buffering-newest full-snapshot subscription on MainActor, preserves scheduler order and the last stopped value, rejects stale generations after restart, and forwards actions without optimistic state. Its immutable row items expose a local basename and only a validated optional `dm://` remote logical path, never a Mac absolute path or Core's raw failure description. The authenticated product session owns a scheduler whose private Application Support manifest is isolated by authenticated device fingerprint. `DroidMatchAppSupport` commits a security-scoped bookmark before submission, reacquires/refreshes it before local file I/O, balances access with a lease, and prunes records no longer referenced by queue state; a FIFO consistency gate keeps registration, enqueue/removal, health reads, and retry reconciliation atomic across actor reentrancy, while transfer I/O remains scheduler-owned and concurrent. Core sees only a platform-neutral access provider. App-sandbox/SAF jobs use one recovery retry, while fresh-only MediaStore creation disables replay. Disconnect pauses recoverable work and retains unsafe work as non-replayable `interrupted`. Slot C archives sandbox-entitled authentication, browsing, bidirectional transfer, and forced-relaunch upload recovery; Developer ID signing/notarization and broader device/provider evidence remain separate release work.
