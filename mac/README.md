...
- `DirectoryMutationClient` / `DirectoryBrowserPolicy` / `DirectoryBrowserModel`：通过 async RPC 在 App Sandbox 或可写 SAF 当前目录创建直接子文件夹；150 行纯策略负责 direct-child 名称/路径、当前已加载条目的 mutation admission、批量稳定排序与有界错误分类，但不持有 client、Task、generation、token、缓存或 Published 状态。574 行 MainActor 模型继续唯一持有异步 mutation，切换目录会取消旧操作，错误状态只保留分类而不保留用户输入名称。
