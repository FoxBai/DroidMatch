...
跨 scheduler 重建的队列持久化是显式 opt-in：调用方创建 `TransferQueuePersistenceStore(fileURL:)`，再通过 `AsyncTransferScheduler.restoring(...)` 重建。manifest 使用版本化 JSON 和原子写，保留稳定 UUID/FIFO；任务从 queued 进入 active 前必须先写盘成功。重建时只有 sidecar 可解码且路径匹配的 download 或 app-sandbox/SAF upload 会变成 paused/resumable；MediaStore active、缺失或损坏的 sidecar 都会保留为不可 resume 的 `interrupted`，避免静默重复上传。Manifest 与 App-owned bookmark registry 共用 private atomic writer：同目录候选文件创建时即为 `0600`，完成写入和同步后才原子替换，已有宽权限父目录也不会产生 chmod 前暴露窗口；失败 bookmark mutation 会回滚内存记录。产品恢复始终先持有 execution latch，再对所有非终态本地 endpoint 验证 bookmark 持久化健康和记录覆盖；损坏/不可读的恢复存储，或对这些已恢复目标为空/不完整的 bookmark archive，均保持 `writeFailed` 且不启动 executor。显式重试会先 load bookmark，再在 execution latch 下 reload/validate/canonicalize manifest，然后按新目标集核对 bookmark 覆盖并对齐 orphan authority，最后才解锁 scheduler；Resume 也经过同一健康守门，不做乐观恢复。harness 保持显式 opt-in；产品 App 已按认证设备隔离 Application Support manifest，通过 App-owned bookmark 恢复 sandbox 文件访问，并在断开会话前暂停或中断队列。
