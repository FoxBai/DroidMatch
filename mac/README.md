...
现有命令行 harness 继续保留同步 `FramedTcpSession`，以维持已归档 M1 结果的行为稳定。新 SwiftUI/AppKit 产品代码不得在主 actor 调用同步 session，也不要用 `Task.detached` 包一层伪异步；控制面与传输流统一通过 `AsyncRpcControlClient`。产品异步层已有本地 TCP 覆盖的原子文件下载 + 四块窗口化上传 + heartbeat 混合路由，并验证上传/下载协议取消后会话仍可复用；下载与上传 sidecar/自动重连现已分别由 `AsyncDownloadCoordinator`、`AsyncUploadCoordinator` 接入，尚未完成的是产品 UI transfer queue 绑定和真机混合流证据。
