...
跨 scheduler 重建的队列持久化是显式 opt-in：调用方创建 `TransferQueuePersistenceStore(fileURL:)`，再通过 `AsyncTransferScheduler.restoring(...)` 重建。manifest 使用版本化 JSON 和原子写，保留稳定 UUID/FIFO；任务从 queued 进入 active 前必须先写盘成功。重建时只有 sidecar 可解码且路径匹配的 download 或 app-sandbox/SAF upload 会变成 paused/resumable；MediaStore active、缺失或损坏的 sidecar 都会保留为不可 resume 的 `interrupted`，避免静默重复上传。Manifest 与 App-owned bookmark registry 共用 private atomic writer：同目录候选文件创建时即为 `0600`，完成写入和同步后才原子替换，已有宽权限父目录也不会产生 chmod 前暴露窗口；失败 bookmark mutation 会回滚内存记录。任一存储在启动加载失败时都会保留原文件、阻止提交或授权访问，并保持 `writeFailed`；显式重试会重新 load/validate/canonicalize durable state 而不是用空运行时状态覆盖，底层状态修复后无需重启或重连即可恢复。产品队列通过同一个重试按钮分别恢复两处存储并重新读取权威状态，不做乐观恢复。harness 保持显式 opt-in；产品 App 已按认证设备隔离 Application Support manifest，通过 App-owned bookmark 恢复 sandbox 文件访问，并在断开会话前暂停或中断队列。
